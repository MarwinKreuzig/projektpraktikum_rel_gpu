...
std::vector<float> johnson(typename Graph::FullGraph& full_graph, const size_t num_neurons, bool use_cuda_if_available) {
...
//    };
//
//    std::vector<float> distances(num_neurons * num_neurons);
//
...
std::vector<float> johnson_parallel(typename Graph::FullGraph& full_graph, size_t num_neurons) {
    ...
    std::vector<int> weights{};
    weights.reserve(full_graph.m_edges.size());
    std::transform(edge_begin_it, edge_end_it, std::back_inserter(weights), [&](const auto& edge) {
        return weight_map(edge);
    });
