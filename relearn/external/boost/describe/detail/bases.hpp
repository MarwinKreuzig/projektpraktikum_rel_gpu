...
namespace describe {
    namespace detail {
        ...
        template <class C, class B>
        constexpr unsigned base_descriptor<C, B>::modifiers;

        template <class... T>
        auto base_descriptor_fn_impl(int, T...) {
            return list<T...>();
        }
...
#define BOOST_DESCRIBE_BASES(C, ...) \
    inline auto boost_base_descriptor_fn(C**) { return boost::describe::detail::base_descriptor_fn_impl(0 BOOST_DESCRIBE_PP_FOR_EACH(BOOST_DESCRIBE_BASE_IMPL, C, __VA_ARGS__)); }
