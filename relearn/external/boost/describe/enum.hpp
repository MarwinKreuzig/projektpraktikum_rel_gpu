...
namespace describe {
    namespace detail {
        ...
        template <class D>
        constexpr decltype(D::value()) enum_descriptor<D>::value;
        template <class D>
        constexpr decltype(D::name()) enum_descriptor<D>::name;

        template <class... T>
        auto enum_descriptor_fn_impl(int, T...) {
            return list<enum_descriptor<T>...>();
        }

#define BOOST_DESCRIBE_ENUM_BEGIN(E) \
    inline auto boost_enum_descriptor_fn(E**) { return boost::describe::detail::enum_descriptor_fn_impl( 0
...
#define BOOST_DEFINE_FIXED_ENUM(E, Base, ...) \
    enum E : Base { __VA_ARGS__ };            \
    BOOST_DESCRIBE_ENUM(E, __VA_ARGS__)
#define BOOST_DEFINE_FIXED_ENUM_CLASS(E, Base, ...) \
    enum class E : Base { __VA_ARGS__ };            \
    BOOST_DESCRIBE_ENUM(E, __VA_ARGS__)
