...
namespace random {
    namespace detail {
        ...
        struct real_alias_table {
            ...
            static void normalize(std::vector<WeightType>& weights) {
                WeightType sum = std::accumulate(weights.begin(), weights.end(),
    ...
    public:
        ...
        result_type max BOOST_PREVENT_MACRO_SUBSTITUTION() const { return static_cast<result_type>(_impl._alias_table.size() - 1); }
    ...
    private:
        ...
        void init(Iter first, Iter last, std::forward_iterator_tag) {
            ...
            while (b_iter != b_end && a_iter != a_end) {
                _impl._alias_table[static_cast<std::size_t>(b_iter->second)] = std::make_pair(b_iter->first, a_iter->second);
            ...
            for (; b_iter != b_end; ++b_iter) {
                _impl._alias_table[static_cast<std::size_t>(b_iter->second)].first = _impl.get_weight(b_iter->second);
            }
            for (; a_iter != a_end; ++a_iter) {
                _impl._alias_table[static_cast<std::size_t>(a_iter->second)].first = _impl.get_weight(a_iter->second);
            }
        }
        template <class Iter>
