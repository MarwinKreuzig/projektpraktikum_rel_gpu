...
namespace random {
    ...
    public:
        ...
        static BOOST_CONSTEXPR result_type max BOOST_PREVENT_MACRO_SUBSTITUTION() { return max_imp(boost::is_integral<UIntType>()); }
        ...
        BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(independent_bits_engine,
            base_result_type, seed_arg) { _base.seed(seed_arg); }
         ...
         */
        template <class It>
        void seed(It& first, It last) { _base.seed(first, last); }
        ...
        result_type operator()() {
            ...
            std::size_t m = (range == (std::numeric_limits<base_unsigned>::max)()) ? std::numeric_limits<base_unsigned>::digits : detail::integer_log2(range + 1);
