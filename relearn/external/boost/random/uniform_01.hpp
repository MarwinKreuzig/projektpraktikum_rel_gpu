    ...
    namespace detail {
        ...
        public:
            ...
            result_type operator()(Engine& eng) {
                for (;;) {
                    ...
                    result_type factor = result_type(1) / (result_type(base_result((eng.max)() - (eng.min)())) + result_type(std::numeric_limits<base_result>::is_integer ? 1 : 0));
        ...
        public:
            ...
            explicit backward_compatible_uniform_01(typename traits::rvalue_type rng)
                : _rng(rng)
                , _factor(result_type(1) / (result_type((base().max)() - (base().min)()) + result_type(std::numeric_limits<base_result>::is_integer ? 1 : 0))) {
            }
            // compiler-generated copy ctor and copy assignment are fine
