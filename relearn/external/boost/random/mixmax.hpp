...
namespace random {
    ...
    public: // SEEDING FUNCTIONS
        template <class It>
        mixmax_engine(It& first, It last) { seed(first, last); }
        ...
        void discard(boost::uint64_t nsteps) {
            for (boost::uint64_t j = 0; j < nsteps; ++j)
                (*this)();
    ...
    template <int Ndim, unsigned int SPECIALMUL, boost::int64_t SPECIAL>
    mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::mixmax_engine(boost::uint64_t seedval) {
    ...
    template <int Ndim, unsigned int SPECIALMUL, boost::int64_t SPECIAL>
    mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::mixmax_engine(uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t streamID) {
        // constructor, four 32-bit seeds for 128-bit seeding flexibility
        seed_uniquestream(&S, clusterID, machineID, runID, streamID);
    }

    template <int Ndim, unsigned int SPECIALMUL, boost::int64_t SPECIAL>
    uint64_t mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::MULWU(uint64_t k) { return (((k) << (SPECIALMUL)&M61) ^ ((k) >> (BITS - SPECIALMUL))); }

    template <int Ndim, unsigned int SPECIALMUL, boost::int64_t SPECIAL>
    boost::uint64_t mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::iterate_raw_vec(boost::uint64_t* Y, boost::uint64_t sumtotOld) {
    ...
    template <int Ndim, unsigned int SPECIALMUL, boost::int64_t SPECIAL>
    void mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::seed_uniquestream(rng_state_t* Xin, uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t streamID) {
        seed_vielbein(Xin, 0);
    ...
    template <int Ndim, unsigned int SPECIALMUL, boost::int64_t SPECIAL>
    boost::uint64_t mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::apply_bigskip(boost::uint64_t* Vout, boost::uint64_t* Vin, uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t streamID) {
        /*
...
#include "boost/random/detail/mixmax_skip_N17.ipp"
    ...
    template <int Ndim, unsigned int SPECIALMUL, boost::int64_t SPECIAL>
    inline boost::uint64_t mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::fmodmulM61(boost::uint64_t cum, boost::uint64_t s, boost::uint64_t a) {
    ...
    template <int Ndim, unsigned int SPECIALMUL, boost::int64_t SPECIAL>
    boost::uint64_t mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::modadd(boost::uint64_t foo, boost::uint64_t bar) {
