...
namespace random {
    ...
    public:
        ...
        static BOOST_CONSTEXPR result_type max BOOST_PREVENT_MACRO_SUBSTITUTION() { return low_bits_mask_t<w>::sig_bits; }
        ...
        BOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(lagged_fibonacci_engine,
            SeedSeq, seq) { seed(seq); }

        /**
         * Creates a new @c lagged_fibonacci_engine and calls @c seed(first, last).
         */
        template <class It>
        lagged_fibonacci_engine(It& first, It last) { seed(first, last); }
         ...
         */
        BOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(lagged_fibonacci_engine, x_, y_) { return x_.i == y_.i && std::equal(x_.x, x_.x + long_lag, y_.x); }
    ...
    public:
        ...
        template <class It>
        lagged_fibonacci_01_engine(It& first, It last) { seed(first, last); }
        ...
        /** Fills a range with random values */
        template <class Iter>
        void generate(Iter first, Iter last) { return detail::generate_from_real(*this, first, last); }
         ...
         * sequences of outputs.
         */
        BOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(lagged_fibonacci_01_engine, x_, y_) { return x_.i == y_.i && std::equal(x_.x, x_.x + long_lag, y_.x); }
