...
namespace mp11 {
    ...
    template <template <class...> class F, class... L>
    using mp_transform = typename mp_if<typename detail::mp_same_size_2<L...>::type, detail::mp_transform_impl<F, L...>, detail::list_size_mismatch>::type;
    ...
    namespace detail {
        ...
        template <template <class...> class F, template <class...> class L1, class... T1, template <class...> class L2, class... T2, template <class...> class L3, class... T3, template <class...> class L4, class... T4, class... L>
    ...
    using mp_transform_if_q = typename detail::mp_transform_if_impl<Qp::template fn, Qf::template fn, L...>::type;
    ...
    template <class L, std::size_t N>
    using mp_drop_c = typename detail::mp_drop_impl<L, mp_repeat_c<mp_list<void>, N>, mp_bool<N <= mp_size<L>::value>>::type;
    ...
    template <class S>
    using mp_from_sequence = typename detail::mp_from_sequence_impl<S>::type;

    // mp_iota(_c)<N>
    template <std::size_t N>
    using mp_iota_c = mp_from_sequence<make_index_sequence<N>>;
    template <class N>
    using mp_iota = mp_from_sequence<make_integer_sequence<typename std::remove_const<decltype(N::value)>::type, N::value>>;
    ...
    template <class L, std::size_t N>
    using mp_take_c = typename detail::mp_take_c_impl<N, L>::type;
    template <class L, class N>
    using mp_take = typename detail::mp_take_c_impl<std::size_t{ N::value }, L>::type;
    ...
    namespace detail {
        ...
        template <template <class...> class L, class T1, class... T, std::size_t I, template <class...> class P>
    ...
    namespace detail {
        ...
        template <template <class...> class L, class T1, class T2, class T3, class T4, class T5, class T6, class T7>
        struct mp_reverse_impl<L<T1, T2, T3, T4, T5, T6, T7>> {
            using type = L<T7, T6, T5, T4, T3, T2, T1>;
        };

        template <template <class...> class L, class T1, class T2, class T3, class T4, class T5, class T6, class T7, class T8>
    ...
    template <class L, std::size_t N>
    using mp_rotate_left_c = detail::mp_rotate_impl<L, detail::canonical_left_rotation<mp_size<L>::value, N>>;
    template <class L, class N>
    ...
    template <class L, class V, template <class...> class F>
    using mp_partial_sum = mp_second<mp_fold_q<L, mp_list<V, mp_clear<L>>, detail::mp_partial_sum_impl_f<F>>>;
    template <class L, class V, class Q>
    ...
    template <class L, class Q>
    using mp_pairwise_fold_q = mp_eval_if<mp_empty<L>, mp_clear<L>, detail::mp_pairwise_fold_impl, L, Q>;
