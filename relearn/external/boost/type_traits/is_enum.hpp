...
namespace detail {
    ...
    struct is_enum_impl {
        ...
        // (dwa 2002/04/15)...
        BOOST_STATIC_CONSTANT(bool, selector = ::boost::is_arithmetic<T>::value || ::boost::is_reference<T>::value || ::boost::is_function<T>::value || is_class_or_union<T>::value || is_array<T>::value);
#else
        // ...however, not checking is_class_or_union on non-conforming
        // compilers prevents a dependency recursion.
        BOOST_STATIC_CONSTANT(bool, selector = ::boost::is_arithmetic<T>::value || ::boost::is_reference<T>::value || ::boost::is_function<T>::value || is_array<T>::value);
