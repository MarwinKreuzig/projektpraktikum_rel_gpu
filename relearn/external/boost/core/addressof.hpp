...
#if !defined(BOOST_NO_CXX11_DECLTYPE) && (defined(__INTEL_COMPILER) || (defined(__clang__) && !defined(_LIBCPP_VERSION)))
...
#if defined(BOOST_NO_CXX11_SFINAE_EXPR) || defined(BOOST_NO_CXX11_CONSTEXPR) || defined(BOOST_NO_CXX11_DECLTYPE)
#define BOOST_CORE_NO_CONSTEXPR_ADDRESSOF

template <class T>
BOOST_FORCEINLINE T*
addressof(T& o) BOOST_NOEXCEPT {
#if BOOST_WORKAROUND(BOOST_BORLANDC, BOOST_TESTED_AT(0x610)) || BOOST_WORKAROUND(__SUNPRO_CC, <= 0x5120)
...
template <class T, std::size_t N>
BOOST_FORCEINLINE const T (*addressof(const T (&o)[N]) BOOST_NOEXCEPT)[N] {
...
namespace detail {
    ...
    template <class T>
    struct addrof_member_operator<T, typename addrof_void<decltype(addrof_declval<T&>().operator&())>::type> {
    ...
    template <class T>
    struct addrof_non_member_operator<T, typename addrof_void<decltype(operator&(addrof_declval<T&>()))>::type> {
        static constexpr bool value = true;
    };
    ...
    struct addrof_is_constexpr {
        static constexpr bool value = addrof_expression<T>::value && !addrof_member_operator<T>::value && !addrof_non_member_operator<T>::value;
    };
