...
#pragma GCC diagnostic ignored "-Wtype-limits"
        ...
        template <class Ch>
        BOOST_CXX14_CONSTEXPR std::size_t find_first_of(Ch const* p_, std::size_t n_, Ch const* s, std::size_t pos, std::size_t n) BOOST_NOEXCEPT {
            ...
            if (use_table) {
                for (std::size_t i = pos; i < n_; ++i) {
                    UCh ch = p_[i];
                    if (ch >= 0 && ch < 256 && table[ch])
                        return i;
                }
            } else if (n >= 16) {
                for (std::size_t i = pos; i < n_; ++i) {
                    Ch ch = p_[i];
                    if (std::char_traits<Ch>::find(s, n, ch) != 0)
                        return i;
                }
            } else {
                for (std::size_t i = pos; i < n_; ++i) {
        ...
        template <class Ch>
        BOOST_CXX14_CONSTEXPR std::size_t find_last_of(Ch const* p_, Ch const* s, std::size_t pos, std::size_t n) BOOST_NOEXCEPT {
            typedef typename sv_to_uchar<Ch>::type UCh;
        ...
        template <class Ch>
        BOOST_CXX14_CONSTEXPR std::size_t find_first_not_of(Ch const* p_, std::size_t n_, Ch const* s, std::size_t pos, std::size_t n) BOOST_NOEXCEPT {
            typedef typename sv_to_uchar<Ch>::type UCh;
        ...
        template <class Ch>
        BOOST_CXX14_CONSTEXPR std::size_t find_last_not_of(Ch const* p_, Ch const* s, std::size_t pos, std::size_t n) BOOST_NOEXCEPT {
            typedef typename sv_to_uchar<Ch>::type UCh;
...
#pragma GCC diagnostic pop
    ...
    public:
        ...
        template <class End>
        BOOST_CXX14_CONSTEXPR basic_string_view(Ch const* first, End last,
            typename boost::enable_if<boost::core::detail::is_same<End, Ch const*>>::type* = 0) BOOST_NOEXCEPT : p_(first),
                                                                                                                 n_(last - first) {
            BOOST_ASSERT(last - first >= 0);
        }

        template <class A>
        basic_string_view(std::basic_string<Ch, std::char_traits<Ch>, A> const& str) BOOST_NOEXCEPT : p_(str.data()), n_(str.size()) {
        }
        ...
        template <class Ch2>
        basic_string_view(boost::basic_string_view<Ch2, std::char_traits<Ch2>> const& str,
            typename boost::enable_if<boost::core::detail::is_same<Ch, Ch2>>::type* = 0) BOOST_NOEXCEPT : p_(str.data()),
                                                                                                          n_(str.size()) {
        }
    ...
    public:
        ...
        template <class A>
        operator std::basic_string<Ch, std::char_traits<Ch>, A>() const {
        ...
        template <class Ch2>
        operator boost::basic_string_view<Ch2,
        ...
        BOOST_CXX14_CONSTEXPR size_type find(Ch const* s, size_type pos, size_type n) const BOOST_NOEXCEPT {
            ...
            for (;;) {
                ...
                if (traits_type::compare(p + 1, s + 1, n - 1) == 0)
                    return p - data();
        ...
        BOOST_CXX14_CONSTEXPR size_type rfind(Ch const* s, size_type pos, size_type n) const BOOST_NOEXCEPT {
            ...
            for (;;) {
                ...
                if (traits_type::compare(data() + xpos, s, n) == 0)
                    return xpos;
        ...
        BOOST_CXX14_CONSTEXPR size_type find_first_not_of(Ch const* s, size_type pos, size_type n) const BOOST_NOEXCEPT {
            if (pos >= size())
                return npos;
            if (n == 1)
                return find_first_not_of(s[0], pos);
        ...
        template <class A>
        BOOST_CXX14_CONSTEXPR friend bool operator==(basic_string_view sv1, std::basic_string<Ch, std::char_traits<Ch>, A> const& sv2) BOOST_NOEXCEPT {
            return sv1.size() == sv2.size() && traits_type::compare(sv1.data(), sv2.data(), sv1.size()) == 0;
        }

        template <class A>
        BOOST_CXX14_CONSTEXPR friend bool operator==(std::basic_string<Ch, std::char_traits<Ch>, A> const& sv1, basic_string_view sv2) BOOST_NOEXCEPT {
            return sv1.size() == sv2.size() && traits_type::compare(sv1.data(), sv2.data(), sv1.size()) == 0;
        }
    ...
    template <class Ch>
    std::basic_ostream<Ch>& operator<<(std::basic_ostream<Ch>& os, basic_string_view<Ch> str) {
        Ch const* p = str.data();
    ...
    template <class Ch>
    BOOST_CONSTEXPR_OR_CONST std::size_t basic_string_view<Ch>::npos;
#endif
