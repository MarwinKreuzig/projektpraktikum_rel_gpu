...
#pragma GCC diagnostic ignored "-Wsign-compare"
    ...
    template <class T>
    inline const T& test_output_impl(const T& v) { return v; }
    ...
    template <class T>
    inline const void* test_output_impl(T volatile* v) { return const_cast<T*>(v); }
    ...
    struct lwt_long_type { };
    template <>
    struct lwt_long_type<true> {
        typedef long type;
    };
    template <>
    struct lwt_long_type<false> {
...
#define BOOST_TEST_THROWS(EXPR, EXCEP)                                                                 \
    try {                                                                                              \
        EXPR;                                                                                          \
        ::boost::detail::throw_failed_impl(#EXPR, #EXCEP, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION); \
    } catch (EXCEP const&) {                                                                           \
        ::boost::detail::test_results();                                                               \
    } catch (...) {                                                                                    \
        ::boost::detail::throw_failed_impl(#EXPR, #EXCEP, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION); \
...
#define BOOST_TEST_NO_THROW(EXPR)                                                                           \
    try {                                                                                                   \
        ...
        ::boost::detail::no_throw_failed_impl(#EXPR, e.what(), __FILE__, __LINE__, BOOST_CURRENT_FUNCTION); \
    } catch (...) {                                                                                         \
        ::boost::detail::no_throw_failed_impl(#EXPR, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION);           \
    }
  //
#else
#define BOOST_TEST_NO_THROW(EXPR) \
