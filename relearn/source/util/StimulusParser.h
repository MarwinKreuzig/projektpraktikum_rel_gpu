...
public:
    ...
    [[nodiscard]] static std::optional<Stimulus> parse_line(const std::string& line, const int my_rank) {
        ...
        for (std::string current_value{}; ss >> current_value;) {
            const auto& rank_neuron_id_vector = FileLoader::split_string(current_value, ':');
            if (rank_neuron_id_vector.size() == 2) {
                ...
                RelearnException::check(!FileLoader::is_number(current_value), "StimulusParser::parseLine:: Illegal neuron id {} in stimulus files. Must have the format <rank>:<neuron_id> or be an area name", current_value);
