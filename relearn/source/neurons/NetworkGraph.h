...
public:
    ...
    NetworkGraph(const number_neurons_type number_neurons, const MPIRank mpi_rank)
            : neuron_distant_in_neighborhood(number_neurons), neuron_distant_out_neighborhood(number_neurons),
              neuron_local_in_neighborhood(number_neurons), neuron_local_out_neighborhood(number_neurons),
              number_local_neurons(number_neurons), my_rank(mpi_rank) {
        RelearnException::check(my_rank.is_initialized(),
                                "NetworkGraph::NetworkGraph: The mpi rank must be initialized");
    }
    ...
    void add_synapse(const LocalSynapse &synapse) {
        ...
        RelearnException::check(local_target_id < number_local_neurons,
                                "NetworkGraph::add_synapse: Local synapse had a too large target: {} vs {}", target,
                                number_local_neurons);
        RelearnException::check(local_source_id < number_local_neurons,
                                "NetworkGraph::add_synapse: Local synapse had a too large source: {} vs {}", source,
                                number_local_neurons);
    ...
    void add_synapse(const DistantInSynapse &synapse) {
        ...
        RelearnException::check(local_target_id < number_local_neurons,
                                "NetworkGraph::add_synapse: Distant in-synapse had a too large target: {} vs {}",
                                target, number_local_neurons);
        RelearnException::check(source_rank != my_rank,
                                "NetworkGraph::add_synapse: Distant in-synapse was on my rank: {}", source_rank);
        RelearnException::check(weight != 0, "NetworkGraph::add_synapse: Local synapse had weight 0");
    ...
    void add_synapse(const DistantOutSynapse &synapse) {
        ...
        RelearnException::check(local_source_id < number_local_neurons,
                                "NetworkGraph::add_synapse: Distant out-synapse had a too large target: {} vs {}",
                                source, number_local_neurons);
        RelearnException::check(target_rank != my_rank,
                                "NetworkGraph::add_synapse: Distant out-synapse was on my rank: {}", target_rank);
        RelearnException::check(weight != 0, "NetworkGraph::add_synapse: Local synapse had weight 0");
     ...
     */
    void add_edges(const LocalSynapses &local_edges, const DistantInSynapses &in_edges,
                   const DistantOutSynapses &out_edges) {
    ...
    [[nodiscard]] std::vector<unsigned int> get_edges_histogram(const EdgeDirection edge_direction) const {
        ...
        const auto &local_neighborhood = (edge_direction == EdgeDirection::In) ? neuron_local_in_neighborhood
                                                                               : neuron_local_out_neighborhood;
        const auto &distant_neighborhood = (edge_direction == EdgeDirection::In) ? neuron_distant_in_neighborhood
                                                                                 : neuron_distant_out_neighborhood;
    ...
    void print_with_ranks(std::ostream &os_out_edges, std::ostream &os_in_edges, const bool flag) const {
        for (const auto &source_id: NeuronID::range(number_local_neurons)) {
            ...
            for (const auto &[target_id, weight]: neuron_local_out_neighborhood[source_local_id]) {
                ...
                os_out_edges << my_rank.get_rank() << ' ' << (target_local_id + 1) << '\t' << my_rank.get_rank() << ' '
                             << (source_local_id + 1) << '\t' << weight << '\t' << flag << '\n';
            }
            ...
            for (const auto &[target_neuron, weight]: neuron_distant_out_neighborhood[source_local_id]) {
                ...
                os_out_edges << target_rank.get_rank() << ' ' << (target_local_id + 1) << '\t' << my_rank.get_rank()
                             << ' ' << (source_local_id + 1) << '\t' << weight << '\t' << flag << '\n';
            }
        }
        ...
        for (const auto &target_id: NeuronID::range(number_local_neurons)) {
            ...
            for (const auto &[source_id, weight]: neuron_local_in_neighborhood[target_local_id]) {
                ...
                os_in_edges << my_rank.get_rank() << ' ' << (target_local_id + 1) << '\t' << my_rank.get_rank() << ' '
                            << (source_local_id + 1) << '\t' << weight << '\t' << flag << '\n';
            }
            ...
            for (const auto &[source_neuron, weight]: neuron_distant_in_neighborhood[target_local_id]) {
                ...
                os_in_edges << my_rank.get_rank() << ' ' << (target_local_id + 1) << '\t' << source_rank.get_rank()
                            << ' ' << (source_local_id + 1) << '\t' << weight << '\t' << flag << '\n';
            }
        }
    }
