...
std::tuple<std::vector<LoadedNeuron>, LoadedNeuronsInfo> NeuronIO::load_neurons_from_file(const std::filesystem::path& file_path) {
    ...
    RelearnException::check(file_is_good && !file_is_not_good, "NeuronIO::load_neurons_from_file: Opening the file was not successful");
    ...
    for (std::string line{}; std::getline(file, line);) {
        ...
        RelearnException::check(pos_x >= 0, "NeuronIO::load_neurons_from_file: x position of neuron {} was negative: {}", id, pos_x);
        RelearnException::check(pos_y >= 0, "NeuronIO::load_neurons_from_file: y position of neuron {} was negative: {}", id, pos_y);
        RelearnException::check(pos_z >= 0, "NeuronIO::load_neurons_from_file: z position of neuron {} was negative: {}", id, pos_z);

        id--;

        RelearnException::check(id == expected_id, "NeuronIO::load_neurons_from_file: Loaded neuron with id {} but expected: {}", id, expected_id);
...
LocalSynapses NeuronIO::read_local_synapses_from_file(const std::filesystem::path& file_path, std::uint64_t number_local_neurons) {
    ...
    for (std::string line{}; std::getline(file_synapses, line);) {
        ...
        RelearnException::check(success, "NeuronIO::read_local_synapses_from_file: Loading synapses was unsuccessfull!");

        RelearnException::check(read_target_id > 0 && read_target_id <= number_local_neurons, "NeuronIO::read_local_synapses_from_file: target_id was not from [1, {}]: {}", number_local_neurons, read_target_id);
        RelearnException::check(read_source_id > 0 && read_source_id <= number_local_neurons, "NeuronIO::read_local_synapses_from_file: source_id was not from [1, {}]: {}", number_local_neurons, read_source_id);

        RelearnException::check(weight != 0, "NeuronIO::read_local_synapses_from_file: weight was 0");
