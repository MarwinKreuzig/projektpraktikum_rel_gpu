...
CalciumIO::load_initial_and_target_function(const std::filesystem::path& path_to_file) {
    ...
    auto initial_calculator = [lookup = std::move(id_to_initial), default_initial = default_initial_calcium]([[maybe_unused]] int mpi_rank, NeuronID::value_type neuron_id) {
    ...
    auto target_calculator = [lookup = std::move(id_to_target), default_target = default_target_calcium]([[maybe_unused]] int mpi_rank, NeuronID::value_type neuron_id) {
        const auto& contains = lookup.find(neuron_id) != lookup.end();
        if (contains) {
