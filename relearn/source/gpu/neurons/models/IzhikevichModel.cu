...
namespace gpu::models {
    __device__ Izhikevich::Izhikevich(const unsigned int _h, gpu::background::BackgroundActivity* bgc, double _V_spike, double _a, double _b, double _c, double _d, double _k1, double _k2, double _k3)
            : NeuronModel(_h, bgc)
            , V_spike(_V_spike)
            , a(_a)
            , b(_b)
    ...
    __device__ void Izhikevich::init(RelearnGPUTypes::number_neurons_type number_neurons) {
    ...
    __device__ void Izhikevich::update_activity(RelearnGPUTypes::step_type step) {
    ...
    __device__ void Izhikevich::create_neurons(RelearnGPUTypes::number_neurons_type creation_count) {
        NeuronModel::create_neurons(creation_count);
        const auto new_size = extra_infos->get_number_local_neurons();
        u.resize(new_size);
    }

    __device__ void Izhikevich::init_neurons(const RelearnGPUTypes::number_neurons_type start_id, const RelearnGPUTypes::number_neurons_type end_id) {
