...
AEIFModelGPU::AEIFModelGPU(const unsigned int h, std::unique_ptr<SynapticInputCalculator>&& synaptic_input_calculator,
    ...
    : NeuronModelGPU(create_aeif_model_data(E_L), h, std::move(synaptic_input_calculator), std::move(background_activity_calculator), std::move(stimulus_calculator)), C { _C}, g_L{_g_L}, E_L{_E_L}, V_T{_V_T}, d_T{_d_T}, tau_w{_tau_w}, a{_a}, b{_b}, V_spike{_V_spike} {
    }
