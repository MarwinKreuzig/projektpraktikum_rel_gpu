...
/**
 * This class defines an interface to access the utils implementation from the host side.
 * It separates the utils code from the host code.
 * Most methods create a handle to control the object on the utils from the here defined methods from the host.
 * The target relearn_lib uses only the definition without utils code as it can be compiled without utils
 * The target relearn_lib_gpu implements this interface and handles all GPU calls
 */

// Macros to define methods as virtual/implemented somewhere else when compiled with utils
...
/**
 * @return Pointer to the class that handles the NeuronExtraInfos on the utils
 */
...
public:
    ...
    /**
     * Copies the background activity from the utils to the cpu.
     * Donot call this method unless it is absolutly necessary as it introduces additional work on memory and the NeuronModel can access the background activity directly from the utils memory.
     */
    ...
    /**
     * Set the NeuronsExtraInfos to the model on the utils
     * @param extra_infos_handle Host handle to the NeuronsExtraInfo instance on the utils
     */
    virtual void set_extra_infos(const std::unique_ptr<gpu::neurons::NeuronsExtraInfosHandle>& extra_infos_handle) = 0;
};

/**
 * Create constant background activity calculator on utils.
 * @param c Constant background activity
 * @return Pointer to the class that handles the BackgroundActivitiy on the utils
 */
std::shared_ptr<BackgroundHandle> set_constant_background(double c) CUDA_PTR_DEFINITION

    /**
     * Create normally distributed background activity calculator on utils.
...
/**
 * Create Izhikevich neuron model on utils.
 ...
 * @return Pointer to the class that handles the NeuronModel on the utils
 */
...
/**
 * Create Poisson neuron model on utils.
 * @param background_handle The host handle to the background activity calculator
...
/**
 * Create AEIF neuron model on utils.
 * @param background_handle The host handle to the background activity calculator
...
/**
 * Create Fitz Hugh Nagumo neuron model on utils.
 * @param background_handle The host handle to the background activity calculator
