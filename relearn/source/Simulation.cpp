...
/**
* @brief Simulatio::startSimulationstarts the Simulation with the given parameters
* @param model NeuronModel to be used 
* @param neuron_ids contains the neuron_ids of neurons that will be observed
* @param argc number of arguments
* @param argv arguments (num_neurons, seed_octree, simulation steps, ...)
*/
void Simulation::startSimulation(std::unique_ptr<NeuronModels> model, std::vector<size_t> neuron_ids, int argc, char** argv)
{
	std::vector<std::string> arguments{ argv, argv + argc };
	/**
	 * Read Arguments (No need for executable command I guess?)
	 */
	if (arguments.size() < 5) {
		std::cout << " <acceptance criterion (theta)>"
			<< " <number neurons>"
			<< " <random number seed>"
			<< " <simulation steps>"
	...
	Simulation::setDefaultParameters(&params);
	 ...
	 */
	Neurons neurons = partition.get_local_neurons(std::move(model), params, *neurons_in_subdomain);
	...
	for (size_t i = 0; i < neuron_ids.size(); i++) {
		monitors.emplace_back(neuron_ids.at(i), neurons);
	}
	...
	x_dims = neurons.get_positions().get_x_dims();
	y_dims = neurons.get_positions().get_y_dims();
	z_dims = neurons.get_positions().get_z_dims();


	this->network_graph = &network_graph;

	//Information are available in MPI process 0.
	information.clear();
	size_t id;
	std::map<size_t, std::vector<NeuronInformation>>::iterator it = information.begin();
	for (auto& m : monitors)
	{
		id = m.get_target_id();
		information.insert(std::pair<size_t, std::vector<NeuronInformation>>(id, m.get_informations()));
	}
}

/**
* @brief Simulation::setParameters reads the parameters from the given neuron model and saves them in a Parameters object
* @param neuron model
* @return returns a Parameters Object with the values of the given neuron model
*/
void Simulation::setParameters(std::unique_ptr<NeuronModels>& model, Parameters* params)
{
	std::string name;

	//Setzt die vorhandenen Modellparameter
	for(auto modelparams : model->get_parameter())
	{
		if (std::holds_alternative<NeuronModels::Parameter<unsigned int>>(modelparams))
		{
			name = std::get<NeuronModels::Parameter<unsigned int>>(modelparams).name();
			if (lut.contains(name))
			{
				unsigned int tmp = std::get<NeuronModels::Parameter<unsigned int>>(modelparams).value();
				lut.setValue(name, std::get<NeuronModels::Parameter<unsigned int>>(modelparams).value());
			}
		}

		if (std::holds_alternative<NeuronModels::Parameter<double>>(modelparams))
		{
			name = std::get<NeuronModels::Parameter<double>>(modelparams).name();
			if (lut.contains(name))
			{
				double tmp = std::get<NeuronModels::Parameter<double>>(modelparams).value();
				lut.setValue(name, std::get<NeuronModels::Parameter<double>>(modelparams).value());
			}
		}

		if (std::holds_alternative<NeuronModels::Parameter<size_t>>(modelparams))
		{
			name = std::get<NeuronModels::Parameter<size_t>>(modelparams).name();
			if (lut.contains(name))
			{
				size_t tmp = std::get<NeuronModels::Parameter<size_t>>(modelparams).value();
				lut.setValue(name, std::get<NeuronModels::Parameter<size_t>>(modelparams).value());
			}
		}
		
	}
	lut.getParameter(params);

	//Die restlichen Parameter werden fürs erste Default gesetzt
	params->frac_neurons_exc = 0.8;
...
/**
* @brief Simulation::setDefaultParameters sets the default parameters for the Simulation
* @param params, the parameters which are going to be set
*/
void Simulation::setDefaultParameters(Parameters* params)
{
...
/**
* @brief Simulaton::setSpecifcParameters sets the specific Parameters for the Simulation
* @param params the parameters that are going to be set
* @param arguments contains the specific parameters that are used to set the specific parameters
*/
void Simulation::setSpecificParameters(Parameters* params, const std::vector<std::string> arguments)
{
	RelearnException::check(params != nullptr);
...
/**
* @brief Simulation::printTimers prints timers and memory usage
*/
void Simulation::printTimers()
{
	/**
...
/**
* @brief Simulation::printNeuronMonitor information of the observed neuron will be printed into a file
* @param nm the used NeuronMonitor
* @param neuron_id the Id of the neuron that is observed
*/
void Simulation::printNeuronMonitor(const NeuronMonitor& nm, size_t neuron_id)
{
