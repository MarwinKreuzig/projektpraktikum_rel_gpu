...
CommunicationMap<SynapseCreationRequest> FastMultipoleMethods::find_target_neurons(const number_neurons_type number_neurons) {
    ...
    if (total_number_dendrites_ex > 0) {
        FastMultipoleMethodsBase::make_creation_request_for(root, local_branch_nodes, branch_level, ElementType::Axon, SignalType::Excitatory, synapse_creation_requests_outgoing);
    }
    if (total_number_dendrites_in > 0) {
        FastMultipoleMethodsBase::make_creation_request_for(root, local_branch_nodes, branch_level, ElementType::Axon, SignalType::Inhibitory, synapse_creation_requests_outgoing);
    }
...
std::pair<CommunicationMap<SynapseCreationResponse>, std::pair<PlasticLocalSynapses, PlasticDistantInSynapses>> FastMultipoleMethods::process_requests(const CommunicationMap<SynapseCreationRequest>& creation_requests) {
    return ForwardConnector::process_requests(creation_requests, excitatory_dendrites, inhibitory_dendrites);
}

PlasticDistantOutSynapses FastMultipoleMethods::process_responses(const CommunicationMap<SynapseCreationRequest>& creation_requests,
