...
void NeuronsExtraInfoTest::assert_empty(const NeuronsExtraInfo &nei, size_t number_neurons) {
    ...
    for (const auto i: NeuronID::range_id(number_neurons_out_of_scope)) {
        ...
        ASSERT_THROW(
        const auto &tmp = nei.get_position(neuron_id), RelearnException) << "assert empty position" << neuron_id;
    }
}

void NeuronsExtraInfoTest::assert_contains(const NeuronsExtraInfo &nei, size_t number_neurons, size_t num_neurons_check,
    ...
    for (auto neuron_id: NeuronID::range(num_neurons_check)) {
        ...
        ASSERT_EQ(expected_positions[neuron_id.get_neuron_id()], actual_positions[neuron_id.get_neuron_id()])
                << neuron_id;
    ...
    for (const auto i: ranges::views::indices(number_neurons_out_of_scope)) {
        ...
        ASSERT_THROW(
        const auto &tmp = nei.get_position(neuron_id), RelearnException) << neuron_id;
    }
}
...
ASSERT_THROW(nei
.
set_positions(std::vector<NeuronsExtraInfo::position_type>{}
), RelearnException);
...
ASSERT_THROW(nei
.
set_positions(std::vector<NeuronsExtraInfo::position_type>(new_size)
), RelearnException);
...
ASSERT_THROW(nei
.
set_positions(positions_wrong), RelearnException
);
...
positions_right[neuron_id] =
SimulationAdapter::get_random_position(mt);
}

nei.
set_positions(positions_right);
...
positions_right_2[neuron_id] =
SimulationAdapter::get_random_position(mt);
}

nei.
set_positions(positions_right_2);
...
nei.
init(num_neurons_init);

ASSERT_THROW(nei
.
create_neurons(num_neurons_create_1), RelearnException
);
...
const auto neuron_id
:
NeuronID::range_id(num_neurons_init)
) {
positions_right[neuron_id] =
SimulationAdapter::get_random_position(mt);
}
...
nei.
create_neurons(num_neurons_create_1);
...
const auto neuron_id
:
NeuronID::range_id(num_neurons_total_1)
) {
positions_right_2[neuron_id] =
SimulationAdapter::get_random_position(mt);
}
...
nei.
create_neurons(num_neurons_create_2);
...
const auto neuron_id
:
NeuronID::range_id(num_neurons_total_2)
) {
positions_right_3[neuron_id] =
SimulationAdapter::get_random_position(mt);
}

nei.
set_positions(positions_right_3);
...
ASSERT_THROW(nei
.
set_enabled_neurons(enabled_neurons), RelearnException
);
ASSERT_NO_THROW(nei
.
set_disabled_neurons(disabled_neurons)
);
ASSERT_NO_THROW(nei
.
...
if (
std::ranges::binary_search(enabled_neurons, neuron_id
)) {
ASSERT_EQ(status_flags[index], UpdateStatus::Enabled
);
ASSERT_TRUE(nei
.
does_update_electrical_actvity(neuron_id)
);
ASSERT_TRUE(nei
.
does_update_plasticity(neuron_id)
);
} else if (
std::ranges::binary_search(disabled_neurons, neuron_id
)) {
...
ASSERT_THROW(nei
.
set_disabled_neurons(disabled_neurons), RelearnException
);
ASSERT_THROW(nei
.
...
nei.
set_disabled_neurons(std::vector{NeuronID(2)}
);
nei.
set_enabled_neurons(std::vector{NeuronID(2)}
);
...
nei.
set_static_neurons(std::vector{NeuronID(2), NeuronID(3)}
);

ASSERT_THROW(nei
.
set_enabled_neurons(std::vector{NeuronID(3)}
), RelearnException);
ASSERT_THROW(nei
.
set_disabled_neurons(std::vector{NeuronID(2)}
...
for (
auto it = 0;
it<number_neurons_for_rank;
it++) {
cm.
emplace_back(rank, NeuronIdAdapter::get_random_neuron_id(number_neurons, mt)
);
}
}
...
cm.
emplace_back(faulty_rank, number_neurons
);
