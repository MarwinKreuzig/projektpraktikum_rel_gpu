...
void NeuronsExtraInfoTest::assert_empty(const NeuronsExtraInfo& nei, size_t number_neurons) {
    ...
    for (const auto i : NeuronID::range_id(number_neurons_out_of_scope)) {
        ...
        ASSERT_THROW(
            const auto& tmp = nei.get_position(neuron_id), RelearnException)
            << "assert empty position" << neuron_id;
    }
}
...
TEST_F(NeuronsExtraInfoTest, testSetStatus) {
    ...
    ASSERT_THROW(nei
                     .set_enabled_neurons(enabled_neurons),
        RelearnException);
    ASSERT_NO_THROW(nei
...
TEST_F(NeuronsExtraInfoTest, testSetStatusOutOfBounds) {
    ...
    enabled_neurons.emplace_back(number_neurons
        + 1);
    disabled_neurons.emplace_back(number_neurons
        + 2);
