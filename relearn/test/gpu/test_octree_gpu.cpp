...
octree.
insert(box_size_type(6, 0, 0), NeuronID{false, 0}
); // vorne rechts unten
octree.
insert(box_size_type(9, 0, 4), NeuronID{false, 1}
); // vorne rechts unten
octree.
insert(box_size_type(0, 0, 6), NeuronID{false, 2}
); // vorne links oben
octree.
insert(box_size_type(4, 0, 9), NeuronID{false, 3}
); // vorne links oben
octree.
insert(box_size_type(4, 0, 4), NeuronID{false, 5}
); // vorne links unten
octree.
insert(box_size_type(9, 0, 9), NeuronID{false, 4}
); // vorne rechts oben
octree.
insert(box_size_type(9, 6, 9), NeuronID{false, 6}
); // hinten rechts oben
octree.
insert(box_size_type(9, 6, 4), NeuronID{false, 7}
); // hinten rechts unten
octree.
insert(box_size_type(4, 6, 4), NeuronID{false, 8}
); // hinten links unten
octree.
insert(box_size_type(4, 6, 7), NeuronID{false, 9}
); // hinten links oben
...
expected.neuron_ids[current_leaf_node_num] = nodes[i]->
...
expected.minimum_cell_position[index] =
gpu::Vec3d(
        std::get<0>(nodes[i]->get_size())
.
...
expected.maximum_cell_position[index] =
gpu::Vec3d(
        std::get<1>(nodes[i]->get_size())
.
...
expected.position_excitatory_element[index] =
convert_vec_to_gpu_vec(nodes[i]
->

get_cell()

.
get_position_for(element_type, SignalType::Excitatory
).

value()

);
expected.position_inhibitory_element[index] =
convert_vec_to_gpu_vec(nodes[i]
->
...
.
get_position_for(element_type, SignalType::Inhibitory
).

value()

);
expected.num_free_elements_excitatory[index] = nodes[i]->

get_cell()

.
get_number_elements_for(element_type, SignalType::Excitatory
);
expected.num_free_elements_inhibitory[index] = nodes[i]->

get_cell()

.
get_number_elements_for(element_type, SignalType::Inhibitory
);
}
...
ASSERT_EQ(result
.neuron_ids, expected.neuron_ids);
ASSERT_EQ(result
.num_children, expected.num_children);

ASSERT_EQ(result
.child_indices, expected.child_indices);

for (
int i = 0;
i<expected.minimum_cell_position.

size();

i++) {
assert_eq_vec(convert_gpu_vec_to_vec(result.minimum_cell_position[i]), convert_gpu_vec_to_vec(
        expected.minimum_cell_position[i])
);
assert_eq_vec(convert_gpu_vec_to_vec(result.maximum_cell_position[i]), convert_gpu_vec_to_vec(
        expected.maximum_cell_position[i])
);
assert_eq_vec(convert_gpu_vec_to_vec(result.position_excitatory_element[i]), convert_gpu_vec_to_vec(
        expected.position_excitatory_element[i])
);
assert_eq_vec(convert_gpu_vec_to_vec(result.position_inhibitory_element[i]), convert_gpu_vec_to_vec(
        expected.position_inhibitory_element[i])
);
}

ASSERT_EQ(result
.num_free_elements_excitatory, expected.num_free_elements_excitatory);
ASSERT_EQ(result
.num_free_elements_inhibitory, expected.num_free_elements_inhibitory);
}
...
octree.
initializes_leaf_nodes(neurons_to_place
.
...
octree.
construct_on_gpu(neurons_to_place
.
...
gpu_handle->
copy_to_cpu(octree_cpu_copy);

ASSERT_EQ(octree_cpu_copy
.neuron_ids.
...
for (
int i = 0;
i<octree_cpu_copy.neuron_ids.

size();

i++) {
ASSERT_EQ(octree_cpu_copy
.neuron_ids[i], octree.get_leaf_nodes()[octree_cpu_copy.neuron_ids[i]]->get_cell_neuron_id().
...
get_number_virtual_neurons()

- 1);

size_t correct_counts = 0;
while (!octree_nodes_cpu.
...
ASSERT_EQ(Vec3d(pos_ex_elem.x, pos_ex_elem.y, pos_ex_elem.z), current_node_cpu
->

get_cell()

.
get_position_for(elem_type, SignalType::Excitatory
).

value()

) << " Correct nodes before fail:" <<
correct_counts;

gpu::Vec3d pos_in_elem = octree_cpu_copy.position_inhibitory_element.at(current_node_gpu);
ASSERT_EQ(Vec3d(pos_in_elem.x, pos_in_elem.y, pos_in_elem.z), current_node_cpu
->

get_cell()

.
get_position_for(elem_type, SignalType::Inhibitory
).
...
ASSERT_EQ(num_ex_elem, current_node_cpu
->

get_cell()

.
get_number_elements_for(elem_type, SignalType::Excitatory
));

RelearnTypes::counter_type num_in_elem = octree_cpu_copy.num_free_elements_inhibitory.at(current_node_gpu);
ASSERT_EQ(num_in_elem, current_node_cpu
->

get_cell()

.
get_number_elements_for(elem_type, SignalType::Inhibitory
));
...
push(child);
octree_nodes_gpu.
push(octree_cpu_copy
.child_indices[
children_processed *gpu_handle
->
...
if (children_processed != octree_cpu_copy.num_children.
at(current_node_gpu
- num_neurons))
RelearnException::fail("Octree::overwrite_cpu_tree_with_gpu: GPU and CPU Octree structure differs");
}
