...
i++) {
golden_numbers[i] =
RandomHolder::get_random_uniform_integer<unsigned int>(RandomHolderKey::Partition,
0, 1000);
}
...
ASSERT_THROW(auto val = RandomHolder::get_random_uniform_indices(RandomHolderKey::Partition, number_indices,
                                                                 number_elements), RelearnException);
...
i++) {
random_numbers[i] =
RandomHolder::get_random_normal_double(RandomHolderKey::Partition, mean, stddev
);
}
