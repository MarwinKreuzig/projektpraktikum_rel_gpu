...
TEST_F(RandomTest, testSeeding) {
    RandomHolder::seed(RandomHolderKey::Partition,
        1234);
    ...
    for (
        ...
        i < iterations;
        i++) {
        golden_numbers[i] = RandomHolder::get_random_uniform_integer<unsigned int>(RandomHolderKey::Partition,
            0, 1000);
    }
    ...
    for (
        ...
        for (
            ...
            i < iterations;
            i++) {
            repeated_numbers[i] = RandomHolder::get_random_uniform_integer<unsigned int>(RandomHolderKey::Partition,
                0, 1000);
        }
...
TEST_F(RandomTest, testUniformIntegerRange) {
    ...
    for (
        ...
        i < iterations;
        i++) {
        const auto random_number = RandomHolder::get_random_uniform_integer(RandomHolderKey::Partition, lower_inclusive,
            upper_inclusive);
...
TEST_F(RandomTest, testUniformDoubleRange) {
    ...
    for (
        ...
        i < iterations;
        i++) {
        const auto random_number = RandomHolder::get_random_uniform_double(RandomHolderKey::Partition, lower_inclusive,
            upper_exclusive);
        ASSERT_LE(lower_inclusive, random_number);
...
TEST_F(RandomTest, testUniformIndices) {
    ...
    for (
        ...
        i < iterations;
        i++) {
        const auto indices = RandomHolder::get_random_uniform_indices(RandomHolderKey::Partition, number_indices,
            number_elements);
        ASSERT_EQ(indices
                      .
...
TEST_F(RandomTest, testUniformIndicesException) {
    ...
    ASSERT_THROW(auto val = RandomHolder::get_random_uniform_indices(RandomHolderKey::Partition, number_indices,
                     number_elements),
        RelearnException);
}
...
TEST_F(RandomTest, testFillException) {
    ...
    std::vector<double> random_numbers(iterations);
    ASSERT_THROW(RandomHolder::fill(RandomHolderKey::Partition, random_numbers, lower_inclusive, upper_exclusive);
                 , RelearnException);
}
