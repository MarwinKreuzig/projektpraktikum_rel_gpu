...
1000), RelearnException) << decay_amount_low << ' ' << 1000;
ASSERT_THROW(CalciumCalculator
cc2(TargetCalciumDecay::Relative, decay_amount_high,
1000), RelearnException) << decay_amount_high << ' ' << 1000;
ASSERT_THROW(CalciumCalculator
cc2(TargetCalciumDecay::Relative,
1.0, 1000), RelearnException) << 0.0 << ' ' << 1000;
ASSERT_THROW(CalciumCalculator
cc3(TargetCalciumDecay::Relative,
0.5, 0), RelearnException) << 0.5 << ' ' << 0;
}
...
ASSERT_THROW(CalciumCalculator
cc1(TargetCalciumDecay::Absolute,
0.0, 100), RelearnException) << 0.0 << ' ' << 100;
ASSERT_THROW(CalciumCalculator
cc3(TargetCalciumDecay::Absolute, decay_amount,
100), RelearnException) << decay_amount << ' ' << 100;
}
...
get_decay_type(), TargetCalciumDecay::None

);
ASSERT_EQ(cc2
.

get_decay_type(), TargetCalciumDecay::Relative

);
ASSERT_EQ(cc3
.

get_decay_type(), TargetCalciumDecay::Absolute
...
get_decay_amount(), decay_amount_relative

);
ASSERT_EQ(cc3
.

get_decay_amount(), decay_amount_absolute
...
get_beta(), CalciumCalculator::default_beta

);
ASSERT_EQ(cc1
.

get_tau_C(), CalciumCalculator::default_tau_C

);
ASSERT_EQ(cc1
.

get_h(), CalciumCalculator::default_h
...
set_h(h);
cc1.
set_initial_calcium_calculator(initiator);
cc1.
set_target_calcium_calculator(calculator);
...
cc1.
set_initial_calcium_calculator(initiator_created);
cc1.
set_target_calcium_calculator(calculator_created);
...
const auto neuron_id
:
NeuronID::range_id(number_neurons, number_neurons
+ number_created_neurons)) {
...
ASSERT_THROW(cc1
.
create_neurons(number_neurons), RelearnException
);
ASSERT_THROW(cc2
.
create_neurons(number_neurons), RelearnException
...
extra_info->
init(update_size
== fired_size ? update_size + 1 : update_size);
...
ASSERT_THROW(cc1
.update_calcium(0, fired_status), RelearnException);
ASSERT_THROW(cc1
.
...
extra_info->
set_disabled_neurons(neuron_ids);
...
* decay_amount_relative, now_target[neuron_id], eps);
...
ASSERT_NEAR(previous_target[neuron_id]
- decay_amount_absolute, now_target[neuron_id], eps);
