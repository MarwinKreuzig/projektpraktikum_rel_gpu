...
node.
set_cell_neuron_position(own_position);
...
std::vector <std::pair<Vec3d, NeuronID>> placed_neurons = OctreeAdapter::template extract_neurons<AdditionalCellAttributes>(
        &node);
...
node.
set_cell_neuron_position(Vec3d{25.0, 25.0, 25.0}
);
...
OctreeNode <AdditionalCellAttributes> node{};
node.
set_cell_neuron_id(id_iterator);
node.
set_rank(rank_iterator);
...
auto *child_2 = node.insert(
        SimulationAdapter::get_random_position_in_box(min, min + midpoint, this->mt) + Vec3d(mid_x, 0, 0), NeuronID(2));
auto *child_3 = node.insert(
        SimulationAdapter::get_random_position_in_box(min, min + midpoint, this->mt) + Vec3d(0, mid_y, 0), NeuronID(3));
auto *child_4 = node.insert(
        SimulationAdapter::get_random_position_in_box(min, min + midpoint, this->mt) + Vec3d(0, 0, mid_z), NeuronID(4));
auto *child_5 = node.insert(
        SimulationAdapter::get_random_position_in_box(min, min + midpoint, this->mt) + Vec3d(0, mid_y, mid_z),
        NeuronID(5));
auto *child_6 = node.insert(
        SimulationAdapter::get_random_position_in_box(min, min + midpoint, this->mt) + Vec3d(mid_x, 0, mid_z),
        NeuronID(6));
auto *child_7 = node.insert(
        SimulationAdapter::get_random_position_in_box(min, min + midpoint, this->mt) + Vec3d(mid_x, mid_y, 0),
        NeuronID(7));
auto *child_8 = node.insert(
        SimulationAdapter::get_random_position_in_box(min, min + midpoint, this->mt) + Vec3d(mid_x, mid_y, mid_z),
        NeuronID(8));
...
if
constexpr (AdditionalCellAttributes::has_excitatory_dendrite) {
        for (auto* child : node.get_children()) {
            const auto vacant_elements = RandomAdapter::get_random_integer<typename OctreeNode<AdditionalCellAttributes>::counter_type>(
                    0, 10, this->mt);
...
if
constexpr (AdditionalCellAttributes::has_inhibitory_dendrite) {
        for (auto* child : node.get_children()) {
            const auto vacant_elements = RandomAdapter::get_random_integer<typename OctreeNode<AdditionalCellAttributes>::counter_type>(
                    0, 10, this->mt);
...
if
constexpr (AdditionalCellAttributes::has_excitatory_axon) {
        for (auto* child : node.get_children()) {
            const auto vacant_elements = RandomAdapter::get_random_integer<typename OctreeNode<AdditionalCellAttributes>::counter_type>(
                    0, 10, this->mt);
...
if
constexpr (AdditionalCellAttributes::has_inhibitory_axon) {
        for (auto* child : node.get_children()) {
            const auto vacant_elements = RandomAdapter::get_random_integer<typename OctreeNode<AdditionalCellAttributes>::counter_type>(
                    0, 10, this->mt);
...
std::unordered_map<OctreeNode <
                   AdditionalCellAttributes>*, typename OctreeNode<AdditionalCellAttributes>::counter_type> vacant_excitatory_dendrites{
};
std::unordered_map<OctreeNode <
                   AdditionalCellAttributes>*, typename OctreeNode<AdditionalCellAttributes>::counter_type> vacant_inhibitory_dendrites{
};
std::unordered_map<OctreeNode <
                   AdditionalCellAttributes>*, typename OctreeNode<AdditionalCellAttributes>::counter_type> vacant_excitatory_axons{
};
std::unordered_map<OctreeNode <
                   AdditionalCellAttributes>*, typename OctreeNode<AdditionalCellAttributes>::counter_type> vacant_inhibitory_axons{
};
...
golden_position_excitatory_dendrites /=
golden_number_excitatory_dendrites;
}

if (golden_number_inhibitory_dendrites != 0) {
golden_position_inhibitory_dendrites /=
golden_number_inhibitory_dendrites;
}

if (golden_number_excitatory_axons != 0) {
golden_position_excitatory_axons /=
golden_number_excitatory_axons;
}

if (golden_number_inhibitory_axons != 0) {
golden_position_inhibitory_axons /=
golden_number_inhibitory_axons;
}
...
idx++) {
ASSERT_EQ(idx
* Constants::number_oct, touched_rmas[idx]);
}
}
...
ASSERT_THROW(auto val = TT::get_all_positions_for(&node, ElementType::Dendrite, SignalType::Excitatory),
             RelearnException);
...
ASSERT_THROW(auto val = TT::get_all_positions_for(&node, ElementType::Dendrite, SignalType::Inhibitory),
             RelearnException);
