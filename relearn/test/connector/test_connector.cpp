...
const auto &excitatory_dendrites = SynapticElementsAdapter::create_dendrites(number_neurons_1, SignalType::Excitatory,
                                                                             mt);
const auto &inhibitory_dendrites = SynapticElementsAdapter::create_dendrites(final_number_neurons,
                                                                             SignalType::Inhibitory, mt);
...
ASSERT_THROW(auto val = ForwardConnector::process_requests(incoming_requests, excitatory_dendrites, empty),
             RelearnException);

ASSERT_THROW(auto val = ForwardConnector::process_requests(incoming_requests, empty, inhibitory_dendrites),
             RelearnException);

ASSERT_THROW(
        auto val = ForwardConnector::process_requests(incoming_requests, excitatory_dendrites, inhibitory_dendrites),
        RelearnException);

CommunicationMap <SynapseCreationResponse> incoming_responses{static_cast<size_t>(number_ranks_1)};

ASSERT_THROW(auto val = ForwardConnector::process_responses(incoming_requests, incoming_responses, empty),
             RelearnException);

CommunicationMap <SynapseCreationResponse> wrong_incoming_responses{static_cast<size_t>(final_number_ranks)};

ASSERT_THROW(auto val = ForwardConnector::process_responses(incoming_requests, wrong_incoming_responses, empty),
             RelearnException);
...
const auto &excitatory_dendrites = SynapticElementsAdapter::create_dendrites(number_neurons, SignalType::Excitatory,
                                                                             mt);
const auto &inhibitory_dendrites = SynapticElementsAdapter::create_dendrites(number_neurons, SignalType::Inhibitory,
                                                                             mt);
...
auto [responses, synapses] = ForwardConnector::process_requests(incoming_requests, excitatory_dendrites,
                                                                inhibitory_dendrites);
auto [local_synapses, distant_in_synapses] = synapses;

ASSERT_EQ(responses
.

size(), incoming_requests
...
get_number_ranks(), incoming_requests
...
ASSERT_TRUE(local_synapses
.

empty()

);
ASSERT_TRUE(distant_in_synapses
.
...
number_excitatory_requests +=
number_vacant_excitatory;

for (
const auto neuron_id
:
NeuronID::range(number_vacant_excitatory)
) {
SynapseCreationRequest scr(id, neuron_id, SignalType::Excitatory);
incoming_requests.
append(MPIRank(1), scr
);

excitatory_requests[id].
emplace_back(scr);
}

const auto number_vacant_inhibitory = inhibitory_dendrites->get_free_elements(id);
number_inhibitory_requests +=
number_vacant_inhibitory;

for (
const auto neuron_id
:
NeuronID::range(number_vacant_inhibitory)
) {
...
ASSERT_EQ(local_synapses
.

size(),

0);
ASSERT_EQ(distant_in_synapses
.

size(), number_excitatory_requests
...
const auto number_vacant_excitatory = excitatory_dendrites->get_free_elements(id);
ASSERT_EQ(number_vacant_excitatory,
0);

const auto number_vacant_inhibitory = inhibitory_dendrites->get_free_elements(id);
ASSERT_EQ(number_vacant_inhibitory,
0);
}
...
ASSERT_EQ(previous_grown_excitatory_counts[neuron_id], now_grown_excitatory_counts[neuron_id]
) <<
neuron_id;
ASSERT_EQ(previous_grown_inhibitory_counts[neuron_id], now_grown_inhibitory_counts[neuron_id]
) <<
neuron_id;

ASSERT_GE(now_connected_excitatory_counts[neuron_id], previous_connected_excitatory_counts[neuron_id]
) <<
neuron_id;
ASSERT_GE(now_connected_inhibitory_counts[neuron_id], previous_connected_inhibitory_counts[neuron_id]
) <<
neuron_id;

ASSERT_LE(now_connected_excitatory_counts[neuron_id],
static_cast
<unsigned int>(now_grown_excitatory_counts[neuron_id])
) <<
neuron_id;
ASSERT_LE(now_connected_inhibitory_counts[neuron_id],
static_cast
<unsigned int>(now_grown_inhibitory_counts[neuron_id])
) <<
neuron_id;
...
ASSERT_EQ(newly_connected_excitatory_dendrites[neuron_id.get_neuron_id()], number_excitatory_requests[neuron_id.get_neuron_id()]
) <<
neuron_id;
}

if (
const auto vacant_inhibitory_elements = inhibitory_dendrites->get_free_elements(neuron_id);
vacant_inhibitory_elements > 0) {
ASSERT_EQ(newly_connected_inhibitory_dendrites[neuron_id.get_neuron_id()], number_inhibitory_requests[neuron_id.get_neuron_id()]
) <<
neuron_id;
}
}
...
if (rank == my_rank) {
expected_local_synapses.
emplace_back(target_index, source_index, weight
);
} else {
expected_distant_in_synapses.
emplace_back(target_index, RankNeuronId{rank, source_index}, weight
);
}
}
}
...
size(), expected_distant_in_synapses
...
const auto neuron_id
:
ranges::views::indices(local_synapses
.
...
const auto neuron_id
:
ranges::views::indices(distant_in_synapses
.
...
ASSERT_EQ(accepted_excitatory_requests[neuron_id], newly_connected_excitatory_dendrites[neuron_id]
) <<
neuron_id;
ASSERT_EQ(accepted_inhibitory_requests[neuron_id], newly_connected_inhibitory_dendrites[neuron_id]
) <<
neuron_id;
}
}
