...
double calculate_excitatory_fraction(const std::vector <SignalType> &types) {
    ...
    const auto ratio =
            static_cast<double>(number_excitatory) / static_cast<double>(number_excitatory + number_inhibitory);
    return ratio;
}
...
const auto rank
:
MPIRank::range(golden_number_ranks)
) {
const auto part = std::make_shared<Partition>(golden_number_ranks, rank);
ASSERT_THROW(SubdomainFromNeuronDensity
sfnd(golden_number_neurons, golden_fraction_excitatory_neurons, golden_um_per_neuron, part
), RelearnException);
}
}
...
ASSERT_NEAR(box_length, golden_box_length,
1.0 / golden_number_neurons);

ASSERT_EQ(0, sfnd.

get_number_placed_neurons()

);
ASSERT_EQ(0.0, sfnd.

get_ratio_placed_excitatory_neurons()
...
ASSERT_NEAR(golden_fraction_excitatory_neurons, requested_fraction_excitatory_neurons,
1.0 / golden_number_neurons);

ASSERT_EQ(requested_number_neurons, placed_number_neurons
);
ASSERT_NEAR(requested_fraction_excitatory_neurons, placed_fraction_excitatory_neurons,
1.0 / golden_number_neurons);
...
ASSERT_EQ(placed_number_neurons, positions
.

size()

);
ASSERT_EQ(placed_number_neurons, types
.

size()

);
ASSERT_EQ(placed_number_neurons, sfnd
.get_local_area_translator()->get_number_neurons_in_total());
}
...
ASSERT_NEAR(placed_ratio_excitatory_neurons, calculated_ratio_excitatory_neurons,
1.0 / golden_number_neurons);
...
.
check_in_box(sim_box_min, sim_box_max
));
...
ASSERT_EQ(number_neurons, lines
.
...
ASSERT_NEAR(x, desired_position
.

get_x(), eps

);
ASSERT_NEAR(y, desired_position
.

get_y(), eps

);
ASSERT_NEAR(z, desired_position
.
...
const auto part = std::make_shared<Partition>(golden_number_ranks, rank);
ASSERT_THROW(SubdomainFromNeuronPerRank
sfnpr(0, golden_fraction_excitatory_neurons, golden_um_per_neuron, part), RelearnException);
}
}
...
ASSERT_EQ(golden_number_neurons
* golden_number_ranks, number_neurons);

ASSERT_NEAR(golden_fraction_excitatory_neurons, fraction_excitatory_neurons,
1.0 / golden_number_neurons);

const auto &[sim_box_min, sim_box_max] = part->get_simulation_box_size();
const auto box_length = (sim_box_max - sim_box_min).get_maximum();

const auto number_neurons_per_box_max = static_cast<size_t>(ceil(
        static_cast<double>(golden_number_neurons) / part->get_number_local_subdomains()));

const auto golden_box_length = calculate_box_length(number_neurons_per_box_max, golden_um_per_neuron) *
                               part->get_number_subdomains_per_dimension();
ASSERT_NEAR(box_length, golden_box_length,
1.0 / golden_number_neurons);
...
ASSERT_NEAR(golden_fraction_excitatory_neurons, placed_ratio_excitatory_neurons,
static_cast
<double>(number_subdomains)
/ golden_number_neurons);
}
}
...
ASSERT_EQ(placed_number_neurons, sfnpr
.get_local_area_translator()->get_number_neurons_in_total());
}
...
const auto placed_number_neurons = sfnpr.get_number_placed_neurons();
accumulated_placed_neurons +=
placed_number_neurons;
...
ASSERT_EQ(placed_number_neurons, all_positions
.

size()

);
ASSERT_EQ(placed_number_neurons, all_types
.
...
ASSERT_EQ(accumulated_placed_neurons, golden_number_ranks
* golden_number_neurons);
}
...
ASSERT_EQ(sfnpr
.get_local_area_translator()->get_number_neurons_in_total(), golden_number_neurons);
}
}
...
ASSERT_NEAR(curr_pos
.

get_x(), curr_loaded_pos
...
ASSERT_TRUE(in_synapses
.

empty()

);
ASSERT_TRUE(out_synapses
.
...
ASSERT_THROW(MultipleSubdomainsFromFile
msff(std::filesystem::path(""),
{
}, partition);, RelearnException);
}
}
...
auto partition = std::make_shared<Partition>(golden_number_ranks, rank);
ASSERT_THROW(MultipleSubdomainsFromFile
msff(std::filesystem::path("./asfhasdfb�aslidhsdjfnasd"),
{
}, partition);, RelearnException);
}
}
