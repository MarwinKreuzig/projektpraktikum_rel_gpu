...
TEST_F(NeuronAssignmentTest, testPerRankTooFewNeurons) {
    ...
    for (
        ...
        ASSERT_THROW(SubdomainFromNeuronPerRank
                         sfnpr(0, golden_fraction_excitatory_neurons, golden_um_per_neuron, part),
            RelearnException);
    }
}
...
TEST_F(NeuronAssignmentTest, testPerRankInitializeMultipleSubdomains) {
    ...
    for (
        ...
        ASSERT_NEAR(golden_fraction_excitatory_neurons, placed_ratio_excitatory_neurons,
            static_cast<double>(number_subdomains)
