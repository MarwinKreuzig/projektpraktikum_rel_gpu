...
template<typename T>
concept verifier_get_rank = requires(T
a) {
{
static_cast<T&>(a).
...
template<typename T>
concept verifier_is_initialized = requires(T
a) {
{
static_cast<T&>(a).
...
static_assert(verifier_get_rank<MPIRank>,
"MPIRank::get_rank() does not return int");
static_assert(verifier_is_initialized<MPIRank>,
"MPIRank::is_initialized() does not return bool");

TEST_F(MPIRankTest, testUnitializedRank
) {
const auto uninitialized_rank = MPIRank::uninitialized_rank();
ASSERT_FALSE(uninitialized_rank
.
...
if (rank_ < 0) {
ASSERT_THROW(MPIRank
rank{
rank_ }, RelearnException) <<
rank_;
