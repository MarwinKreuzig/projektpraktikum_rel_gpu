...
std::vector <Vec3u> actual_indices_vector =
        MultiIndex::get_indices() | ranges::to_vector | ranges::actions::sort(std::less{});
...
ASSERT_NEAR(multiplied, FastMultipoleMethodBase::h(alpha, t), eps
) << t << ' ' << alpha << '\n';
}
...
pointer_index<number_pointers + number_nullptrs +
number_neurons_out_of_scope;
pointer_index++) {
auto *ptr = FastMultipoleMethodBase::extract_element(pointers, pointer_index);
...
node_1.
set_cell_neuron_position(SimulationAdapter::get_random_position_in_box(min, max, this->mt)
);
...
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&node_1, &node_2,
...
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&node_2, &node_1,
                                                                                           ElementType::Dendrite,
                                                                                           SignalType::Excitatory)
);
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&node_2, &node_1,
...
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&node_1, &root,
                                                                                           ElementType::Dendrite,
                                                                                           SignalType::Excitatory)
);
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&node_1, &root,
...
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&root, &node_1,
                                                                                           ElementType::Dendrite,
                                                                                           SignalType::Excitatory)
);
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&root, &node_1,
...
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&node_2, &root,
                                                                                           ElementType::Dendrite,
                                                                                           SignalType::Excitatory)
);
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&node_2, &root,
...
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&root, &node_2,
                                                                                           ElementType::Dendrite,
                                                                                           SignalType::Excitatory)
);
ASSERT_EQ(CalculationType::Direct, FastMultipoleMethodBase::check_calculation_requirements(&root, &node_2,
...
auto check_combi = [&](const ElementType e, const SignalType s) {
    const auto has_enough_in_source =
            source_cell.get_number_elements_for(get_other_element_type(e), s) > Constants::max_neurons_in_source;
...
ASSERT_EQ(FastMultipoleMethodBase::calc_taylor_coefficients(&no_axon_tree, other_position, ElementType::Axon,
                                                            SignalType::Excitatory), coefficients
);
ASSERT_EQ(FastMultipoleMethodBase::calc_taylor_coefficients(&no_axon_tree, other_position, ElementType::Axon,
                                                            SignalType::Inhibitory), coefficients
);
...
ASSERT_EQ(FastMultipoleMethodBase::calc_taylor_coefficients(&no_synaptic_elements_tree, other_position,
                                                            ElementType::Axon, SignalType::Excitatory), coefficients
);
ASSERT_EQ(FastMultipoleMethodBase::calc_taylor_coefficients(&no_synaptic_elements_tree, other_position,
                                                            ElementType::Axon, SignalType::Inhibitory), coefficients
);
ASSERT_EQ(FastMultipoleMethodBase::calc_taylor_coefficients(&no_synaptic_elements_tree, other_position,
                                                            ElementType::Dendrite, SignalType::Excitatory), coefficients
);
ASSERT_EQ(FastMultipoleMethodBase::calc_taylor_coefficients(&no_synaptic_elements_tree, other_position,
                                                            ElementType::Dendrite, SignalType::Inhibitory), coefficients
);
}
