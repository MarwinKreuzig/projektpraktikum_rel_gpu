...
get_level_of_branch_nodes(), level_of_branch_nodes

);
ASSERT_EQ(octree
.

get_simulation_box_minimum(), min

);
ASSERT_EQ(octree
.

get_simulation_box_maximum(), max
...
for (
const auto &id
: virtual_neurons | ranges::views::values) {
level_to_count[id]++;
}
...
get_num_local_trees(), expected_elements
