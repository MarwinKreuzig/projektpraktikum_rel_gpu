...
auto checker = [](std::string_view description, MPIRank default_rank) {
    ASSERT_THROW(auto
    opt_rni = NeuronIdParser::parse_description(description, default_rank);, RelearnException);
};
...
const std::vector <NeuronID> golden_ids =
        rank_neuron_ids | ranges::views::filter(equal_to(my_rank), &RankNeuronId::get_rank) |
...
get_neuron_id(), unique_and_filtered[i + 1]
...
+ position, NeuronID(true, virtual_rma));
