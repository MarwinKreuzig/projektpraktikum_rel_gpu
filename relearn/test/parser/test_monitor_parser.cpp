...
!= my_rank) {
ss << ';' <<
RankNeuronIdAdapter::codify_rank_neuron_id(rni);
continue;
}
...
i<num_regex_areas;
i++) {
area_names.push_back("REG" +
RandomAdapter::get_random_string(RandomAdapter::get_random_integer(1, 20, mt), mt
) + "EX");
}
...
area_id++) {
regex_neurons += translator->
get_number_neurons_in_area(area_id);
}
