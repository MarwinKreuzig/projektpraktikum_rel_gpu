...
v_1.
calculate_componentwise_maximum(v_2);
...
v_1.
calculate_componentwise_minimum(v_2);
...
v_floored = v.floor_componentwise(), RelearnException
) << ss.
...
ASSERT_NEAR(v_normed, root, eps
) << ss.

str();

ASSERT_NEAR(v_normed_squared, sum, eps
) << ss.
...
v.
round_to_larger_multiple(multiple);
...
ASSERT_NO_THROW(std::ignore = w.check_in_box(v, v)
) << ss.
