...
static void write_stimuli_to_file(std::filesystem::path path,
                                  std::vector <std::tuple<RelearnTypes::step_type, RelearnTypes::step_type, RelearnTypes::step_type, double,
...
const auto i
:
ranges::views::indices(num_stimuli)
) {
Interval interval = intervals[i];

const auto intensity = RandomAdapter::get_random_double(0.001, 100.0, mt);
const auto &ids = NeuronIdAdapter::get_random_neuron_ids(num_neurons, RandomAdapter::get_random_integer(
        RelearnTypes::number_neurons_type(1), num_neurons, mt), mt);
...
rank_ids.
insert(std::to_string(rank.get_rank())
+ ":" +
...
stimuli.
emplace_back(std::make_tuple(interval.begin, interval.end, 1U, intensity, rank_ids)
);
my_stimuli.
emplace_back(std::make_tuple(interval.begin, interval.end, 1U, intensity, my_ids)
);
}
...
RelearnTypes::stimuli_function_type stimulus_function = InteractiveNeuronIO::load_stimulus_interrupts(path, my_rank,
...
read_stimulated_neurons += neuron_ids.
...
std::unordered_set <NeuronID> my_ids{};
for (
const auto &area_name
: chosen_area_names) {
for (
const auto &neuron_id
: local_area_translator->
get_neuron_ids_in_area(local_area_translator
->
get_area_id_for_area_name(area_name)
)) {
my_ids.
insert(neuron_id);
}
...
stimuli.
emplace_back(std::make_tuple(interval.begin, interval.end, 1U, intensity,
                             chosen_area_names | ranges::to < std::unordered_set > )
);
my_stimuli.
emplace_back(std::make_tuple(interval.begin, interval.end, 1U, intensity, my_ids)
);
...
{
std::make_tuple(begin, end, frequency, intensity, rank_ids
) });

RelearnTypes::stimuli_function_type stimulus_function = InteractiveNeuronIO::load_stimulus_interrupts(path, MPIRank(0),
                                                                                                      local_area_translator);
for (
auto step = 0U;
step<num_steps;
...
if (
step<begin || step> end
|| (step - begin) % frequency != 0) {
ASSERT_EQ(0, read_stimuli.

size()

);
} else {
ASSERT_EQ(1, read_stimuli.
...
const auto &ids = (i % 2 == 0) ? NeuronIdAdapter::get_random_neuron_ids(num_neurons, RandomAdapter::get_random_integer(
        RelearnTypes::number_neurons_type(1), num_neurons, mt), mt) : std::unordered_set < NeuronID > {};
std::unordered_set <std::string> rank_ids{};
for (
const auto &neuron_id
: ids) {
...
my_stimuli.
emplace_back(std::make_tuple(interval.begin, interval.end, 1U, intensity, ids)
);
}
...
const auto local_area_translator = NeuronAssignmentAdapter::get_randomized_area_translator(mt);
std::filesystem::path path = "stimulus.tmp";
ASSERT_THROW(std::ignore = InteractiveNeuronIO::load_stimulus_interrupts(path, MPIRank(0),
...
write_stimuli_to_file(path,
{
std::make_tuple(interval
.begin, interval.end, 1U, intensity, rank_ids) });
...
const auto invalid_area_name = NeuronAssignmentAdapter::get_invalid_area_name(
        local_area_translator->get_all_area_names(), mt);
const auto num_steps = RandomAdapter::get_random_integer(1000U, 100000U, mt);
const auto intensity = RandomAdapter::get_random_double(0.001, 100.0, mt);
Interval interval = StimulusAdapter::get_random_interval(num_steps, 1U, mt);
...
{
std::make_tuple(interval
.begin, interval.end, 1U, intensity, std::unordered_set{
invalid_area_name }) });
