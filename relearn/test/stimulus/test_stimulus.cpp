...
TEST_F(StimulusTest, testStimulusWithAreas) {
    ...
    for (
        ...
        std::unordered_set<NeuronID> my_ids{};
        for (
            const auto& area_name : chosen_area_names) {
            for (
                const auto& neuron_id : local_area_translator->get_neuron_ids_in_area(local_area_translator
                                                                                          ->get_area_id_for_area_name(area_name))) {
                my_ids.insert(neuron_id);
            }
        }
        stimuli.emplace_back(std::make_tuple(interval.begin, interval.end, 1U, intensity,
...
TEST_F(StimulusTest, testFrequency) {
    ...
    for (
        const auto& neuron_id : ids) {
        rank_ids.insert("0:" + std::to_string(neuron_id.
...
TEST_F(StimulusTest, testInvalidNeuronId) {
    ...
    write_stimuli_to_file(path,
        { std::make_tuple(interval
                              .begin,
            interval.end, 1U, intensity, rank_ids) });
...
TEST_F(StimulusTest, testInvalidAreaName) {
    ...
    write_stimuli_to_file(path,
        { std::make_tuple(interval
                              .begin,
            interval.end, 1U, intensity, std::unordered_set{ invalid_area_name }) });
