...
octree->
initializes_leaf_nodes(number_neurons);
...
barnes_hut.
set_synaptic_elements(axons, excitatory_dendrites, inhibitory_dendrites
);
barnes_hut.
set_neuron_extra_infos(extra_infos);
...
const auto disabled_neurons = NeuronID::range(number_neurons)
                              | ranges::views::filter(equal_to(UpdateStatus::Disabled),
                                                      lookup(update_status, &NeuronID::get_neuron_id))
                              | ranges::to_vector;

extra_infos->
set_disabled_neurons(disabled_neurons);

ASSERT_NO_THROW(barnes_hut
.
...
get_excitatory_dendrites_position()
...
get_inhibitory_dendrites_position()
...
get_number_excitatory_dendrites(),

0);
ASSERT_EQ(cell
.

get_number_inhibitory_dendrites(),
...
get_number_excitatory_dendrites(), golden_excitatory_dendrites

);
ASSERT_EQ(cell
.

get_number_inhibitory_dendrites(), golden_inhibitory_dendrites
...
total_number_excitatory_dendrites +=
number_excitatory_dendrites;
total_number_inhibitory_dendrites +=
number_inhibitory_dendrites;
...
excitatory_dendrites_position += (
position *number_excitatory_dendrites
);
}
...
inhibitory_dendrites_position += (
position *number_inhibitory_dendrites
);
}
...
ASSERT_EQ(total_number_excitatory_dendrites, cell
.

get_number_excitatory_dendrites()

);
ASSERT_EQ(total_number_inhibitory_dendrites, cell
.
