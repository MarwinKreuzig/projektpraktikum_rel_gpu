...
TEST_F(NetworkGraphTest, testNetworkGraphEdges) {
        ...
        if (is_in_synapse) {
            ...
            if (in_edges[my_neuron_id.
                ...
                in_edges[my_neuron_id.
                    ...
                    .erase({ other_rank, other_neuron_id });
            }
        } else {
    ...
    for (
        ...
        for (
            const auto& it : in_edges[neuron_id.
        ...
        for (
            const auto& it : out_edges[neuron_id.

                                       get_neuron_id()

        ]) {
            out_edges_count_meta += std::abs(it
                                                 .second);
        }
...
TEST_F(NetworkGraphTest, testNetworkGraphEdgesSplit) {
    ...
    for (
        ...
        for (
            const auto& val : in_edges_ng_in) {
            in_edges_ng_ex.emplace_back(val);
        }

        for (
            const auto& val : out_edges_ng_ex) {
