...
void NetworkGraphTest::assert_distant_plastic_empty(const NetworkGraph &network_graph) {
    ...
    for (const auto neuron_id: NeuronID::range(number_neurons)) {
        ...
        const auto &incoming_excitatory_partners = network_graph.get_all_plastic_partners_incoming(neuron_id,
                                                                                                   SignalType::Excitatory);
        const auto &incoming_inhibitory_partners = network_graph.get_all_plastic_partners_incoming(neuron_id,
...
void NetworkGraphTest::assert_plastic_size(const NetworkGraph &network_graph,
                                           RelearnTypes::number_neurons_type expected_number_neurons) {
    const auto number_neurons = network_graph.get_number_neurons();
    ...
    for (const auto neuron_id: NeuronID::range(number_neurons, number_neurons + number_neurons_out_of_scope)) {
        ASSERT_THROW(
        const auto &incoming_excitatory_partners = network_graph.get_all_plastic_partners_incoming(neuron_id,
                                                                                                   SignalType::Excitatory);, RelearnException);
        ASSERT_THROW(
        const auto &incoming_inhibitory_partners = network_graph.get_all_plastic_partners_incoming(neuron_id,
                                                                                                   SignalType::Inhibitory);, RelearnException);
        ASSERT_THROW(
        const auto &outgoing_partners = network_graph.get_all_plastic_partners_outgoing(neuron_id);, RelearnException);
    }
}

void NetworkGraphTest::assert_static_size(const NetworkGraph &network_graph,
                                          RelearnTypes::number_neurons_type expected_number_neurons) {
    const auto number_neurons = network_graph.get_number_neurons();
    ASSERT_EQ(number_neurons, expected_number_neurons);
...
ASSERT_THROW(NetworkGraph

ng_exception (MPIRank::uninitialized_rank());
...
ASSERT_THROW(ng
.init(0), RelearnException);

assert_plastic_empty(ng);
assert_static_empty(ng);

assert_plastic_size(ng,
0);
assert_static_size(ng,
0);
...
ASSERT_THROW(ng
.
init(number_neurons), RelearnException
);
...
NetworkGraph ng(rank);
ng.
init(number_neurons_init);
ng.
create_neurons(number_neurons_create);
...
ASSERT_THROW(ng
.create_neurons(0), RelearnException);
...
ASSERT_THROW(ng
.
create_neurons(number_neurons_create), RelearnException
);
...
ng.
add_synapse(PlasticLocalSynapse(target_id, source_id, weight)
);
...
ASSERT_EQ(local_in_edges
.

size(), golden_local_in_edges

);
ASSERT_EQ(distant_in_edges
.

size(),

0);
ASSERT_EQ(local_out_edges
.

size(), golden_local_out_edges

);
ASSERT_EQ(distant_out_edges
.
...
const auto &all_in_edges_excitatory = NetworkGraphAdapter::get_all_plastic_in_edges(ng, MPIRank::root_rank(), neuron_id,
                                                                                    SignalType::Excitatory);
const auto &all_in_edges_inhibitory = NetworkGraphAdapter::get_all_plastic_in_edges(ng, MPIRank::root_rank(), neuron_id,
                                                                                    SignalType::Inhibitory);
const auto &all_out_edges_excitatory = NetworkGraphAdapter::get_all_plastic_out_edges(ng, MPIRank::root_rank(),
                                                                                      neuron_id,
                                                                                      SignalType::Excitatory);
const auto &all_out_edges_inhibitory = NetworkGraphAdapter::get_all_plastic_out_edges(ng, MPIRank::root_rank(),
                                                                                      neuron_id,
                                                                                      SignalType::Inhibitory);

ASSERT_EQ(all_in_edges_excitatory
.

size()

+ all_in_edges_inhibitory.

size(), golden_local_in_edges

);
ASSERT_EQ(all_out_edges_excitatory
.

size()

+ all_out_edges_inhibitory.
...
if (is_in_synapse) {
ng.
add_synapse(PlasticDistantInSynapse(my_neuron_id, other_id, weight)
);
in_edges[my_neuron_id.

get_neuron_id()

][{
other_rank, other_neuron_id }] +=
weight;

if (in_edges[my_neuron_id.

get_neuron_id()

][{
other_rank, other_neuron_id }] == 0) {
in_edges[my_neuron_id.
...
ng.
add_synapse(PlasticDistantOutSynapse(other_id, my_neuron_id, weight)
);
out_edges[my_neuron_id.
...
if (it.second > 0) {
exc_in_edges_count_meta += it.
second;
} else {
inh_in_edges_count_meta += -it.
second;
}
}
...
for (const auto &[key, weight_meta] : in_edges[neuron_id.
...
for (const auto &[key, weight_meta] : out_edges[neuron_id.
...
distant_in_synapses.
emplace_back(my_neuron_id, other_id, weight
);
} else {
ng_golden.
add_synapse(PlasticDistantOutSynapse(other_id, my_neuron_id, weight)
);
distant_out_synapses.
emplace_back(other_id, my_neuron_id, weight
);
}
}
...
local_synapses.
emplace_back(target_id, source_id, weight
);
}

ng.
add_edges(local_synapses, distant_in_synapses, distant_out_synapses
);
...
if (is_in_synapse) {
ng.
add_synapse(PlasticDistantInSynapse(neuron_id, other_id, weight)
);
} else {
ng.
add_synapse(PlasticDistantOutSynapse(other_id, neuron_id, weight)
);
}
}
...
auto in_edges_ng_ex = NetworkGraphAdapter::get_all_plastic_in_edges(ng, MPIRank::root_rank(), neuron_id,
                                                                    SignalType::Excitatory);
const auto &in_edges_ng_in = NetworkGraphAdapter::get_all_plastic_in_edges(ng, MPIRank::root_rank(), neuron_id,
                                                                           SignalType::Inhibitory);
const auto &out_edges_ng_ex = NetworkGraphAdapter::get_all_plastic_out_edges(ng, MPIRank::root_rank(), neuron_id,
                                                                             SignalType::Excitatory);
auto out_edges_ng_in = NetworkGraphAdapter::get_all_plastic_out_edges(ng, MPIRank::root_rank(), neuron_id,
                                                                      SignalType::Inhibitory);

ASSERT_EQ(in_edges_ng
.
...
ASSERT_EQ(out_edges_ng
.

size(), out_edges_ng_ex
...
std::vector <std::tuple<NeuronID, MPIRank, NeuronID, MPIRank, RelearnTypes::plastic_synapse_weight>> synapses(
        num_edges);
...
ng.
add_synapse(PlasticLocalSynapse(neuron_id, other_neuron_id, weight)
);
synapses[edge_id] =
std::make_tuple(neuron_id, MPIRank::root_rank(), other_neuron_id, MPIRank::root_rank(), weight
);
continue;
}
...
synapses[edge_id] =
std::make_tuple(neuron_id, MPIRank::root_rank(), other_neuron_id, other_rank, weight
);
} else {
ng.
add_synapse(PlasticDistantOutSynapse(other_id, neuron_id, weight)
...
synapses[edge_id] =
std::make_tuple(other_neuron_id, other_rank, neuron_id, MPIRank::root_rank(), weight
);
}
}
...
ng.
add_synapse(PlasticLocalSynapse(target_neuron_id, source_neuron_id, -weight)
);
continue;
}
...
ng.
add_synapse(PlasticDistantOutSynapse(target_id, source_neuron_id, -weight)
);
continue;
}

ng.
add_synapse(PlasticDistantInSynapse(target_neuron_id, source_id, -weight)
);
}
...
ASSERT_EQ(exc_in_edges_count_1,
0);
ASSERT_EQ(inh_in_edges_count_1,
0);
ASSERT_EQ(out_edges_count_1,
0);
ASSERT_EQ(exc_in_edges_count_2,
0);
ASSERT_EQ(inh_in_edges_count_2,
0);
ASSERT_EQ(out_edges_count_2,
0);
...
in_edges[my_id][other_id] +=
weight;
} else {
ng.
add_synapse(PlasticDistantOutSynapse(other_id, neuron_id, weight)
);
out_edges[my_id][other_id] +=
weight;
}
}
...
ng.
create_neurons(num_new_neurons);

for (
size_t edge_id = num_edges;
edge_id<total_num_edges;
edge_id++) {
const auto other_rank = MPIRankAdapter::get_random_mpi_rank(32, MPIRank::root_rank(), mt);
const auto neuron_id = NeuronIdAdapter::get_random_neuron_id(number_neurons, mt);
const auto other_neuron_id = NeuronIdAdapter::get_random_neuron_id(number_neurons, mt);
...
auto neuron_id
:
NeuronID::range(total_number_neurons)
) {
const auto [exc_in_edges_count_ng, _1] = ng.get_number_excitatory_in_edges(neuron_id);
const auto [inh_in_edges_count_ng, _2] = ng.get_number_inhibitory_in_edges(neuron_id);
const auto [out_edges_count_ng, _3] = ng.get_number_out_edges(neuron_id);
...
for (const auto &[key, weight_meta] : in_edges[{
MPIRank::root_rank(), neuron_id
...
for (const auto &[key, weight_meta] : out_edges[{
MPIRank::root_rank(), neuron_id
