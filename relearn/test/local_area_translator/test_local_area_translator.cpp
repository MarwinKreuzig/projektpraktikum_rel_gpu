...
ASSERT_EQ(cp_neuron_id_to_area_id[neuron_id.get_neuron_id()], translator
.
get_area_id_for_neuron_id(neuron_id
.

get_neuron_id()

));
ASSERT_EQ(area_id_to_area_name[cp_neuron_id_to_area_id[neuron_id.get_neuron_id()]], translator
.
get_area_name_for_neuron_id(neuron_id
.
...
ASSERT_EQ(cp_area_id_to_area_name[area_id], translator
.
get_area_name_for_area_id(area_id)
);
}
}
...
ASSERT_THROW(LocalAreaTranslator(std::vector<RelearnTypes::area_name>({}),
                                 std::vector<RelearnTypes::area_id>({})), RelearnException
);
