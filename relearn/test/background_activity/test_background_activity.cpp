...
void test_background_exceptions(const std::unique_ptr <BackgroundActivityCalculator> &background_calculator) {
    ...
    for (const auto &neuron_id: NeuronID::range_id(number_neurons) | ranges::views::transform(plus(number_neurons))) {
        ASSERT_THROW(
        const auto input = background_calculator->get_background_activity(NeuronID{neuron_id}), RelearnException);
    }
}

void test_init_create(const std::unique_ptr <BackgroundActivityCalculator> &background_calculator,
                      const size_t number_init_neurons, const size_t number_create_neurons, std::mt19937 &mt,
                      const bool check_input = true, const bool check_equality = true) {
...
const auto constant_background = RandomAdapter::get_random_double<double>(
        BackgroundActivityCalculator::min_base_background_activity,
        BackgroundActivityCalculator::max_base_background_activity, mt);

std::unique_ptr <BackgroundActivityCalculator> background_calculator = std::make_unique<ConstantBackgroundActivityCalculator>(
        constant_background);
...
min(), BackgroundActivityCalculator::min_base_background_activity

);
ASSERT_EQ(param1
.

max(), BackgroundActivityCalculator::max_base_background_activity

);
ASSERT_EQ(param1
.

value(), constant_background
...
const auto mean_background = RandomAdapter::get_random_double<double>(
        BackgroundActivityCalculator::min_background_activity_mean,
        BackgroundActivityCalculator::max_background_activity_mean, mt);
const auto stddev_background = RandomAdapter::get_random_double<double>(
        BackgroundActivityCalculator::min_background_activity_stddev,
        BackgroundActivityCalculator::max_background_activity_stddev, mt);

std::unique_ptr <BackgroundActivityCalculator> background_calculator = std::make_unique<NormalBackgroundActivityCalculator>(
        mean_background, stddev_background);
...
min(), BackgroundActivityCalculator::min_background_activity_mean

);
ASSERT_EQ(param1
.

max(), BackgroundActivityCalculator::max_background_activity_mean

);
ASSERT_EQ(param1
.

value(), mean_background
...
min(), BackgroundActivityCalculator::min_background_activity_stddev

);
ASSERT_EQ(param2
.

max(), BackgroundActivityCalculator::max_background_activity_stddev

);
ASSERT_EQ(param2
.

value(), stddev_background
...
std::unique_ptr <BackgroundActivityCalculator> background_calculator = std::make_unique<FastNormalBackgroundActivityCalculator>(
        mean_background, stddev_background, 5);
...
const auto check_equality = !CudaHelper::is_cuda_available();

const auto number_neurons = NeuronIdAdapter::get_random_number_neurons(mt);
background_calculator->
init(number_neurons);
...
update_status[neuron_id] =
UpdateStatus::Disabled;
extra_info->
set_disabled_neurons(std::vector{NeuronID{neuron_id}}
);
}
}
...
ASSERT_EQ(background_input[neuron_id],
0.0);
}
}
...
if (
std::abs(summed_background)
>=
eps *number_enabled_neurons
) {
std::cerr << "The total variance was: " <<
std::abs(summed_background)
<< '\n';
...
const auto background_values = NeuronID::range_id(number_neurons)
        ...
        [&background_calculator, mean_background](const auto neuron_id) {
            return background_calculator->get_background_activity(NeuronID(neuron_id)) - mean_background;
        })
                               | ranges::to_vector;
...
get_random_calculator(std::mt19937
& mt) {
...
return
std::make_pair(fmt::format("constant:{}", level), params
);
}
if (p == 2) {
...
return
std::make_pair(fmt::format("normal:{},{}", mean, std), params
);
}
...
std::vector <std::tuple<RelearnTypes::step_type,
        std::vector < std::pair < std::string, double>>, std::vector <NeuronID>>>
gold;
...
auto local_neurons = neurons | ranges::view::filter([my_rank](const auto &rni) { return rni.get_rank() == my_rank; }) |
                     ranges::view::transform([](const auto &rni) { return rni.get_neuron_id(); })
                     | ranges::to_vector;
...
gold.
emplace_back(step, params, local_neurons
);
}
of.
...
auto flexible_calculator = std::make_unique<FlexibleBackgroundActivityCalculator>(file_path, my_rank,
...
while (cur_index<gold.

size() &&

std::get<0>(gold[cur_index])
<= step) {
...
const auto calculator_params = flexible_calculator->get_background_activity_calculator_for_neuron(
        neuron_id)->get_parameter();
...
i++) {
ASSERT_NEAR(std::get<Parameter < double>>
(calculator_params[i]).
...
const auto num_selected_neurons = RandomAdapter::get_random_integer<size_t>(1, num_neurons, mt);
const auto &neurons =
        NeuronIdAdapter::get_random_rank_neuron_ids(num_neurons, 1, num_selected_neurons, mt) | ranges::to_vector;
...
gold.
emplace_back(step, params, my_ids
);
}
of.
...
ASSERT_EQ(0, flexible_calculator->
get_background_activity(neuron_id)
);
}
flexible_calculator->
update_input(step,
...
ASSERT_THROW(std::make_unique<FlexibleBackgroundActivityCalculator>(file_path, my_rank,
                                                                    local_area_translator), RelearnException
);
}
...
ASSERT_EQ(flexible_calculator
->
get_background_activity_calculator_for_neuron(neuron_id), last_calculator
);
} else {
