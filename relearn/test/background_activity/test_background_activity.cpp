...
TEST_F(BackgroundActivityTest, testConstantBackgroundActivityConstruct) {
    ...
    ASSERT_EQ(param1
              ...
              max(),
        BackgroundActivityCalculator::max_base_background_activity
...
TEST_F(BackgroundActivityTest, testNormalBackgroundActivityConstruct) {
    ...
    ASSERT_EQ(param1
              ...
              max(),
        BackgroundActivityCalculator::max_background_activity_mean
    ...
    ASSERT_EQ(param2
              ...
              max(),
        BackgroundActivityCalculator::max_background_activity_stddev
...
TEST_F(BackgroundActivityTest, testNormalBackgroundActivityUpdate) {
    ...
    for (
        ...
        if (update_status[neuron_id] == UpdateStatus::Disabled) {
            ...
            background_values.emplace_back(bg
                - mean_background);
...
get_random_calculator(std::mt19937& mt) {
    ...
    for (
        ...
        for (
            ...
            for (
                ...
                i++) {
                ASSERT_NEAR(std::get<Parameter<double>>(calculator_params[i]).
...
TEST_F(BackgroundActivityTest, testFlexibleSameCalculator) {
    ...
    for (
        ...
        for (
            auto neuron_id : affected_neurons) {
            const auto calculator_params = flexible_calculator->get_background_activity_calculator_for_neuron(
                                                                  neuron_id)
                                               ->get_parameter();
            for (
            ...
            if (last_calculator != nullptr) {
                ASSERT_EQ(flexible_calculator
                              ->get_background_activity_calculator_for_neuron(neuron_id),
                    last_calculator);
            } else {
