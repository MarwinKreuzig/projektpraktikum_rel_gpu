...
TEST_F(NeuronModelsTest, testNeuronModelsInitPoisson) {
    ...
    for (auto i = 0; i < iterations; i++) {
        ...
        for (auto j = 0; j < iterations; j++) {
            const auto get_number_requested_neurons = uid_num_neurons(mt);

            model->init(get_number_requested_neurons);
            ...
            ASSERT_EQ(get_number_requested_neurons, number_neurons);
            ASSERT_EQ(get_number_requested_neurons, x.size());
            ASSERT_EQ(get_number_requested_neurons, fired.size());
...
TEST_F(NeuronModelsTest, testNeuronModelsCreateNeuronsPoisson) {
...

    {
        ...
        for (auto j = 0; j < 3; j++) {
            const auto get_number_requested_neurons = uid_num_neurons(mt);
            current_num_neurons += get_number_requested_neurons;

            model->create_neurons(get_number_requested_neurons);
...
TEST_F(NeuronModelsTest, testNeuronModelsUpdateActivityDisabledPoisson) {
    ...
    for (auto i = 0; i < iterations; i++) {
        ...
        const auto empty_graph = generate_random_network_graph(get_number_requested_neurons, 0, 1.0, mt);

        model->init(get_number_requested_neurons);

        std::vector<char> disable_flags(get_number_requested_neurons);
        std::vector<double> model_x = model->get_x();
        std::vector<double> model_secondary(get_number_requested_neurons);
        std::vector<char> model_fired = model->get_fired();
        std::vector<double> model_I_sync(get_number_requested_neurons);

        for (unsigned int id = 0; id < get_number_requested_neurons; id++) {
        ...
        const auto random_graph = generate_random_network_graph(get_number_requested_neurons, get_number_requested_neurons, 1.0, mt);
...
TEST_F(NeuronModelsTest, testNeuronModelsUpdateActivityEnabledNoBackgroundPoisson) {
    ...
    for (auto i = 0; i < iterations; i++) {
        ...
        std::vector<char> disable_flags(get_number_requested_neurons, 1);
        std::vector<double> model_x = model->get_x();
        std::vector<double> model_secondary(get_number_requested_neurons);
        std::vector<char> model_fired = model->get_fired();
