...
ASSERT_THROW(NeuronIO::write_neurons_componentwise(correct_ids, correct_position,
                                                   std::make_shared<LocalAreaTranslator>(correct_area_names,
                                                                                         correct_area_ids),
                                                   faulty_signal_types, path), RelearnException
);
ASSERT_THROW(NeuronIO::write_neurons_componentwise(correct_ids, correct_position,
                                                   std::make_shared<LocalAreaTranslator>(correct_area_names,
                                                                                         faulty_area_ids),
                                                   correct_signal_types, path), RelearnException
);
...
preliminary_area_names.emplace_back("area_" +
...
ASSERT_THROW(NeuronIO::write_neurons_componentwise(preliminary_ids, preliminary_position,
                                                   std::make_shared<LocalAreaTranslator>(preliminary_area_names,
                                                                                         preliminary_area_ids),
                                                   preliminary_signal_types, path), RelearnException
);
}
...
preliminary_ids.emplace_back(false, neuron_id);
preliminary_position.
emplace_back(SimulationAdapter::get_random_position_in_box(min_pos, max_pos, mt)
);
preliminary_area_names.emplace_back("area_" +
std::to_string(neuron_id)
);
...
NeuronIO::write_neurons_componentwise(preliminary_ids, preliminary_position, std::make_shared<LocalAreaTranslator>(
        preliminary_area_names, preliminary_area_ids), preliminary_signal_types, path
);
...
: read_positions) {
minimum.
calculate_componentwise_minimum(position);
maximum.
calculate_componentwise_maximum(position);
}
...
preliminary_neurons.
emplace_back(preliminary_position[neuron_id], preliminary_ids[neuron_id], preliminary_signal_types[neuron_id], preliminary_area_ids[neuron_id]
);
}
auto rng = std::default_random_engine{};
shuffle(preliminary_area_names, rng
...
ASSERT_THROW(NeuronIO::write_neurons(preliminary_neurons, path,
                                     std::make_shared<LocalAreaTranslator>(preliminary_area_names,
                                                                           preliminary_area_ids)), RelearnException
);
}
...
preliminary_neurons.
emplace_back(SimulationAdapter::get_random_position_in_box(min_pos, max_pos, mt), NeuronID{false,
                                                                                           neuron_id}, NeuronTypesAdapter::get_random_signal_type(
        mt), neuron_id
);
area_names.emplace_back("area_" +
std::to_string(neuron_id)
);
area_ids.
emplace_back(neuron_id);
...
.id, preliminary_neuron.id);
ASSERT_EQ(read_neuron
.signal_type, preliminary_neuron.signal_type);
ASSERT_EQ(read_neuron
.area_id, preliminary_neuron.area_id);

minimum.
calculate_componentwise_minimum(read_neuron
.pos);
maximum.
calculate_componentwise_maximum(read_neuron
.pos);
...
preliminary_area_names.emplace_back("area_" +
std::to_string(number_neurons
+ 1));
preliminary_area_ids.
emplace_back(number_neurons);
preliminary_signal_types.
...
if (plastic) {
preliminary_local_synapses_plastic.
emplace_back(target_id, source_id, plastic_weight
);
} else {
preliminary_local_synapses_static.
emplace_back(target_id, source_id, static_weight
);
}
} else {
if (plastic) {
preliminary_distant_synapses_plastic.
emplace_back(target_id, RankNeuronId(source_rank, source_id), plastic_weight
);
} else {
preliminary_distant_synapses_static.
emplace_back(target_id, RankNeuronId(source_rank, source_id), static_weight
);
}
}
...
size(), read_local_synapses_static
...
size(), read_distant_synapses_static
...
size(), read_local_synapses_plastic
...
size(), read_distant_synapses_plastic
...
if (plastic) {
preliminary_distant_synapses_plastic.
emplace_back(RankNeuronId(target_rank, target_id), source_id, plastic_weight
);
} else {
preliminary_distant_synapses_static.
emplace_back(RankNeuronId(target_rank, target_id), source_id, static_weight
);
}
}
...
i++) {
correct_ids.
push_back(NeuronID{i}
);
correct_position.
emplace_back(SimulationAdapter::get_random_position_in_box(sim_box, mt)
);
correct_area_names.emplace_back("area_" +
std::to_string(i)
);
correct_area_ids.
emplace_back(i);
...
NeuronIO::write_neurons_componentwise(correct_ids, correct_position, std::make_shared<LocalAreaTranslator>(
        correct_area_names, correct_area_ids), correct_signal_types, path, total_neurons, sim_box, subdomain_boxes
);
...
ASSERT_EQ(infos
.local_neurons, number_neurons);
ASSERT_TRUE(sim_box
.
equals_eps(infos
.sim_size));
ASSERT_EQ(subdomain_boxes, infos
.subdomain_sizes);
}