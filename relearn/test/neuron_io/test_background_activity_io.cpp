...
std::vector <std::tuple<RelearnTypes::step_type, std::string, std::vector < NeuronID>>>
gold;
...
for (
auto i = 0;
i<num_fake_area_names;
i++) {
of <<
NeuronAssignmentAdapter::get_random_area_name(mt)
<< " ";
}
...
for (
auto i = 0;
i<num_real_area_names;
i++) {
of << real_area_names[i] << " ";
const auto &neurons_in_area = local_area_translator->get_neuron_ids_in_area(
        local_area_translator->get_area_id_for_area_name(real_area_names[i]));
std::copy(neurons_in_area
.

begin(), neurons_in_area

.

end(), std::back_inserter(local_neurons)
...
local_neurons.
erase(unique(local_neurons.begin(), local_neurons.end()), local_neurons
.
...
gold.
emplace_back(step, p, local_neurons
);
}
of.
