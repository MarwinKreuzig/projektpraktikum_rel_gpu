...
ASSERT_NEAR(gaussian_growth_curve(left_intersection, left_intersection, right_intersection, growth_factor),
0.0, eps) << ss.

str();

ASSERT_NEAR(gaussian_growth_curve(right_intersection, left_intersection, right_intersection, growth_factor),
0.0, eps) << ss.

str();

ASSERT_NEAR(gaussian_growth_curve(middle, left_intersection, right_intersection, growth_factor), growth_factor, eps
) << ss.
...
const auto i
:
ranges::views::indices(number_values)
) {
smaller_negatives[i] = RandomAdapter::get_random_double<double>(-100000.0, left_intersection, mt);
smaller_positives[i] =
RandomAdapter::get_random_double<double>(left_intersection, middle, mt
);
larger_positives[i] =
RandomAdapter::get_random_double<double>(middle, right_intersection, mt
);
larger_negatives[i] =
RandomAdapter::get_random_double<double>(right_intersection,
100000.0, mt);
}
...
ASSERT_LE(last_change, current_change
) << ss.
...
ASSERT_GE(growth_factor, current_change
) << ss.
...
ASSERT_GE(last_change, current_change
) << ss.
...
ASSERT_LE(current_change, growth_factor
) << ss.
...
ASSERT_LE(current_change, last_change
) << ss.
...
ASSERT_EQ(gaussian_growth_curve(intersection, intersection, intersection, growth_factor),
0.0);
...
right_values[i] =
RandomAdapter::get_random_double<double>(intersection,
100000.0, mt);
}
...
ASSERT_EQ(gaussian_growth_curve(current_val_left, intersection, intersection, growth_factor),
-growth_factor);
}
...
ASSERT_EQ(gaussian_growth_curve(current_val_right, intersection, intersection, growth_factor),
-growth_factor);
}
}
...
const auto &calcium_to_grow = RandomAdapter::get_random_double<double>(SynapticElements::min_min_C_level_to_grow,
                                                                       SynapticElements::max_min_C_level_to_grow, mt);
const auto &nu = RandomAdapter::get_random_double<double>(SynapticElements::min_nu, SynapticElements::max_nu, mt);
const auto &retract_ratio = RandomAdapter::get_random_double<double>(SynapticElements::min_vacant_retract_ratio,
                                                                     SynapticElements::max_vacant_retract_ratio, mt);
const auto &vacant_elements_lb = RandomAdapter::get_random_double<double>(
        SynapticElements::min_vacant_elements_initially, SynapticElements::max_vacant_elements_initially, mt);
const auto &vacant_elements_ub = RandomAdapter::get_random_double<double>(
        SynapticElements::min_vacant_elements_initially, SynapticElements::max_vacant_elements_initially, mt);
...
SynapticElements synaptic_elements(element_type, calcium_to_grow, nu, retract_ratio, vacant_elements_lb,
                                   vacant_elements_ub);
...
ASSERT_EQ(element_type, synaptic_elements
.
...
ASSERT_EQ(param_min_C
.

value(), calcium_to_grow
...
ASSERT_EQ(param_lower_bound
.

value(), vacant_elements_lb

) << ss.

str();

ASSERT_EQ(param_upper_bound
.

value(), vacant_elements_ub
...
min(), SynapticElements::min_min_C_level_to_grow
...
max(), SynapticElements::max_min_C_level_to_grow
...
value(), SynapticElements::default_nu
...
min(), SynapticElements::min_vacant_retract_ratio
...
value(), SynapticElements::default_vacant_retract_ratio
...
max(), SynapticElements::max_vacant_retract_ratio
...
min(), SynapticElements::min_vacant_elements_initially
...
value(), SynapticElements::default_vacant_elements_initially_lower_bound
...
max(), SynapticElements::max_vacant_elements_initially
...
value(), SynapticElements::default_vacant_elements_initially_upper_bound
...
const auto &d1 = RandomAdapter::get_random_double<double>(SynapticElements::min_min_C_level_to_grow,
                                                          SynapticElements::max_min_C_level_to_grow, mt);
const auto &d2 = RandomAdapter::get_random_double<double>(SynapticElements::min_nu, SynapticElements::max_nu, mt);
const auto &d3 = RandomAdapter::get_random_double<double>(SynapticElements::min_vacant_retract_ratio,
                                                          SynapticElements::max_vacant_retract_ratio, mt);
const auto &d4 = RandomAdapter::get_random_double<double>(SynapticElements::min_vacant_elements_initially,
                                                          SynapticElements::max_vacant_elements_initially, mt);
const auto &d5 = RandomAdapter::get_random_double<double>(SynapticElements::min_vacant_elements_initially,
                                                          SynapticElements::max_vacant_elements_initially, mt);
...
ASSERT_EQ(cloned_param_lower_bound
.
...
ASSERT_EQ(cloned_param_upper_bound
.
...
ASSERT_EQ(synaptic_elements
.

get_size(), number_neurons
...
<<
neuron_id;
ASSERT_EQ(connected_grown_element,
0.0) << ss.
...
:
ranges::views::indices(number_neurons_out_of_scope)
) {
const auto neuron_id = NeuronIdAdapter::get_random_neuron_id(number_neurons, number_neurons, mt);

ASSERT_THROW(auto ret = synaptic_elements.get_grown_elements(neuron_id), RelearnException)
...
ASSERT_THROW(auto ret = synaptic_elements.get_connected_elements(neuron_id), RelearnException)
...
ASSERT_THROW(auto ret = synaptic_elements.get_delta(neuron_id), RelearnException)
...
ASSERT_EQ(connected_grown_elements
.
...
ASSERT_EQ(delta_grown_elements
.
...
for (
const auto &connected_grown_element
: connected_grown_elements) {
ASSERT_EQ(connected_grown_element,
0.0) << ss.
...
for (
const auto &delta_grown_element
: delta_grown_elements) {
ASSERT_EQ(delta_grown_element,
0.0) << ss.
...
SynapticElements synaptic_elements(element_type, 0.0);
synaptic_elements.
init(number_neurons_initially);
synaptic_elements.
create_neurons(number_neurons_added);
...
ASSERT_EQ(grown_elements_1, grown_elements_2
) << ss.
...
<<
neuron_id;
ASSERT_EQ(grown_elements_1, bound
) << ss.
...
ASSERT_THROW(synaptic_elements
.
init(number_neurons_initially), RelearnException
) << ss.
...
ASSERT_THROW(synaptic_elements
.
create_neurons(number_neurons_added), RelearnException
) << ss.
...
param_lower_bound.
set_value(lower_bound_2);
param_upper_bound.
set_value(upper_bound_2);
...
auto neuron_id
:
NeuronID::range(number_neurons_initially)
) {
...
const auto neuron_id
:
NeuronID::range_id(number_neurons_initially, number_neurons
)) {
...
signal_types[neuron_id] =
signal_type;
golden_signal_types[neuron_id] =
signal_type;
}

synaptic_elements.
set_signal_types(std::move(signal_types)
);
...
ASSERT_EQ(synaptic_elements
.
get_signal_type(neuron_id), golden_signal_types[neuron_id.get_neuron_id()]
) << ss.
...
const auto &connected_grown_element = SynapticElementsAdapter::get_random_synaptic_element_connected_count(
        static_cast<unsigned int>(grown_element), mt);
const auto &signal_type = NeuronTypesAdapter::get_random_signal_type(mt);

ASSERT_THROW(synaptic_elements
.
update_grown_elements(neuron_id, grown_element
), RelearnException) << ss.
...
ASSERT_THROW(synaptic_elements
.
update_connected_elements(neuron_id,
static_cast
<int>(connected_grown_element)
), RelearnException) << ss.
...
ASSERT_THROW(synaptic_elements
.
set_signal_type(neuron_id, signal_type
), RelearnException) << ss.
...
golden_counts[neuron_id.

get_neuron_id()

] +=
grown_element;
golden_connected_counts[neuron_id.

get_neuron_id()

] +=
connected_grown_element;
golden_signal_types[neuron_id.
...
synaptic_elements.
update_connected_elements(neuron_id, connected_grown_element
);
synaptic_elements.
set_signal_type(neuron_id, signal_type
);
...
ASSERT_EQ(golden_counts[neuron_id.get_neuron_id()], synaptic_elements
.
get_grown_elements(neuron_id)
) << ss.
...
<<
neuron_id;
ASSERT_EQ(golden_counts[neuron_id.get_neuron_id()], grown_elements[neuron_id.get_neuron_id()]
) << ss.
...
ASSERT_EQ(golden_connected_counts[neuron_id.get_neuron_id()], synaptic_elements
.
get_connected_elements(neuron_id)
) << ss.
...
<<
neuron_id;
ASSERT_EQ(golden_connected_counts[neuron_id.get_neuron_id()], connected_grown_elements[neuron_id.get_neuron_id()]
) << ss.
...
ASSERT_EQ(golden_signal_types[neuron_id.get_neuron_id()], synaptic_elements
.
get_signal_type(neuron_id)
) << ss.
...
<<
neuron_id;
ASSERT_EQ(golden_signal_types[neuron_id.get_neuron_id()], signal_types[neuron_id.get_neuron_id()]
) << ss.
...
ASSERT_EQ(expected_number_free_elements_cast, synaptic_elements
.
get_free_elements(neuron_id)
) << ss.
...
ASSERT_EQ(expected_number_free_elements_cast, synaptic_elements
.
get_free_elements(neuron_id, SignalType::Excitatory
)) << ss.
...
<<
neuron_id;
ASSERT_EQ(0, synaptic_elements.
get_free_elements(neuron_id, SignalType::Inhibitory
)) << ss.
...
synaptic_elements.
update_after_deletion(changes, disabled_neurons
);
...
ASSERT_EQ(synaptic_elements
.
get_connected_elements(neuron_id), golden_connected_counts[neuron_id.get_neuron_id()]
) << ss.
...
ASSERT_EQ(synaptic_elements
.
get_grown_elements(neuron_id), golden_counts[neuron_id.get_neuron_id()]
) << ss.
...
const auto new_connected_count =
        golden_connected_counts[neuron_id.get_neuron_id()] - changes[neuron_id.get_neuron_id()];
...
ASSERT_EQ(synaptic_elements
.
get_connected_elements(neuron_id), new_connected_count
) << ss.
...
ASSERT_THROW(synaptic_elements
.
update_after_deletion(wrong_changes, disabled_neurons
), RelearnException) << ss.
...
] += golden_connected_counts[faulty_id.
...
auto [synaptic_elements, golden_counts, golden_connected_counts, golden_signal_types]
        = SynapticElementsAdapter::create_random_synaptic_elements(number_neurons, element_type,
                                                                   minimum_calcium_to_grow, mt, growth_factor);
...
target_calcium[neuron_id] =
RandomAdapter::get_random_double<double>(minimum_calcium_to_grow, minimum_calcium_to_grow
+ 200.0, mt);
if (
RandomAdapter::get_random_bool(mt)
) {
disable_flags[neuron_id] =
UpdateStatus::Disabled;
extra_info->
set_disabled_neurons(std::vector<NeuronID>{NeuronID{neuron_id}}
);
}
}

synaptic_elements.
update_number_elements_delta(calcium, target_calcium
);
...
const auto computed_delta = gaussian_growth_curve(calcium[id.get_neuron_id()], minimum_calcium_to_grow,
                                                  target_calcium[id.get_neuron_id()], growth_factor);
...
<<
id;
ASSERT_NEAR(actual_deltas[id.get_neuron_id()],
0.0, eps) << ss.
...
ASSERT_NEAR(actual_delta, computed_delta, eps
) << ss.
...
<<
id;
ASSERT_NEAR(actual_deltas[id.get_neuron_id()], computed_delta, eps
) << ss.
...
const auto current_expected_delta = gaussian_growth_curve(calcium[neuron_id], minimum_calcium_to_grow,
                                                          target_calcium[neuron_id], growth_factor);
golden_delta_counts[neuron_id] +=
current_expected_delta;
} else {
extra_info->
set_disabled_neurons(std::vector<NeuronID>{NeuronID{neuron_id}}
);
...
ASSERT_NEAR(actual_delta, expected_delta, eps
) << ss.
...
<<
neuron_id;
ASSERT_NEAR(actual_deltas[neuron_id.get_neuron_id()], expected_delta, eps
) << ss.
...
const auto number_too_large_calcium = RandomAdapter::get_random_integer<size_t>(number_neurons + 1,
                                                                                number_neurons * 2 + 1, mt);

const auto number_too_small_target_calcium = RandomAdapter::get_random_integer<size_t>(1, number_neurons - 1, mt);
const auto number_too_large_target_calcium = RandomAdapter::get_random_integer<size_t>(number_neurons + 1,
                                                                                       number_neurons * 2 + 1, mt);

const auto number_too_small_disable_flags = RandomAdapter::get_random_integer<size_t>(1, number_neurons - 1, mt);
const auto number_too_large_disable_flags = RandomAdapter::get_random_integer<size_t>(number_neurons + 1,
                                                                                      number_neurons * 2 + 1, mt);
...
extra_info_too_small->
init(number_too_small_disable_flags);
...
extra_info_too_large->
init(number_too_large_disable_flags);
...
const auto &connected_grown_element = RandomAdapter::get_random_integer<unsigned int>(0,
                                                                                      static_cast<unsigned int>(grown_element),
                                                                                      mt);
const auto &signal_type = NeuronTypesAdapter::get_random_signal_type(mt);
...
<unsigned int>(connected_grown_element);
golden_signal_types[neuron_id.
...
synaptic_elements.
set_extra_infos(extra_info_all_enabled);
synaptic_elements.
update_number_elements_delta(calcium, target_calcium
);
synaptic_elements.
...
const auto computed_delta = gaussian_growth_curve(calcium[neuron_id.get_neuron_id()], minimum_calcium_to_grow,
                                                  target_calcium[neuron_id.get_neuron_id()], growth_factor);
const auto delta = synaptic_elements.get_delta(neuron_id);
if (disable_flags[neuron_id.

get_neuron_id()

] == UpdateStatus::Disabled) {
ASSERT_NEAR(delta, computed_delta, eps
) << ss.
...
<<
neuron_id;
ASSERT_NEAR(deltas[neuron_id.get_neuron_id()], computed_delta, eps
) << ss.
...
<<
neuron_id;
ASSERT_EQ(deltas[neuron_id.get_neuron_id()],
0.0) << ss.
...
for (
auto deleted_counts
: deleted_element_counts) {
summed_number_deletions +=
deleted_counts;
}

ASSERT_EQ(summed_number_deletions, number_deleted_elements
) << ss.
...
ASSERT_NEAR(expected_count, current_count, eps
) << ss.
...
<<
neuron_id;
ASSERT_EQ(previous_connected, current_connected
) << ss.
...
ASSERT_NEAR(current_count, expected_count, eps
) << ss.
...
ASSERT_EQ(expected_connected, current_connected
) << ss.
...
<<
neuron_id;
ASSERT_EQ(expected_deletions, deleted_element_counts[neuron_id.get_neuron_id()]
) << ss.
...
const auto expected_count =
        (1 - retract_ratio) * (previous_count + computed_delta - expected_connected) + expected_connected;
ASSERT_NEAR(current_count, expected_count, eps
) << ss.
...
const auto number_too_small_disable_flags =
        NeuronIdAdapter::get_random_neuron_id(number_neurons - 1, mt).get_neuron_id() + 1;
const auto number_too_large_disable_flags =
        NeuronIdAdapter::get_random_neuron_id(number_neurons, mt).get_neuron_id() + number_neurons + 1;
...
synaptic_elements.
set_extra_infos(extra_info_too_small);

ASSERT_THROW(auto ret = synaptic_elements.commit_updates(), RelearnException)
...
<<
number_too_small_disable_flags;

synaptic_elements.
set_extra_infos(extra_info_too_large);
...
<<
number_too_large_disable_flags;
}
