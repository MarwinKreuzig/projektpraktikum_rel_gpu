...
TEST_F(SynapticElementsTest, testGaussianGrowthCurve) {
    ...
    ASSERT_NEAR(gaussian_growth_curve(middle, left_intersection, right_intersection, growth_factor), growth_factor, eps) << ss.
    ...
    for (
        ...
        larger_negatives[i] = RandomAdapter::get_random_double<double>(right_intersection,
            100000.0, mt);
    }
    ...
    for (
        ...
        ASSERT_LE(last_change, current_change) << ss.
    ...
    for (
        ...
        ASSERT_GE(growth_factor, current_change) << ss.
    ...
    for (
        ...
        ASSERT_GE(last_change, current_change) << ss.
        ...
        ASSERT_LE(current_change, growth_factor) << ss.
    ...
    for (
        ...
        ASSERT_LE(current_change, last_change) << ss.
...
TEST_F(SynapticElementsTest, testGaussianGrowthCurveSameIntersections) {
    ...
    for (
        ...
        right_values[i] = RandomAdapter::get_random_double<double>(intersection,
            100000.0, mt);
    }
...
TEST_F(SynapticElementsTest, testParameters) {
    ...
    ASSERT_EQ(param_min_C
              ...
              min(),
        SynapticElements::min_min_C_level_to_grow
    ...
    ASSERT_EQ(param_min_C
              ...
              max(),
        SynapticElements::max_min_C_level_to_grow
    ...
    ASSERT_EQ(param_vacant
              ...
              min(),
        SynapticElements::min_vacant_retract_ratio
    ...
    ASSERT_EQ(param_vacant
              ...
              value(),
        SynapticElements::default_vacant_retract_ratio
    ...
    ASSERT_EQ(param_vacant
              ...
              max(),
        SynapticElements::max_vacant_retract_ratio
    ...
    ASSERT_EQ(param_lower_bound
              ...
              min(),
        SynapticElements::min_vacant_elements_initially
    ...
    ASSERT_EQ(param_lower_bound
              ...
              value(),
        SynapticElements::default_vacant_elements_initially_lower_bound
    ...
    ASSERT_EQ(param_lower_bound
              ...
              max(),
        SynapticElements::max_vacant_elements_initially
    ...
    ASSERT_EQ(param_upper_bound
              ...
              value(),
        SynapticElements::default_vacant_elements_initially_upper_bound
...
TEST_F(SynapticElementsTest, testInitialize) {
    ...
    for (
        const auto& grown_element : grown_elements) {
        ASSERT_EQ(grown_element,
            0.0)
            << ss.
    ...
    for (
        const auto& connected_grown_element : connected_grown_elements) {
        ASSERT_EQ(connected_grown_element,
            0.0)
            << ss.
    ...
    for (
        const auto& delta_grown_element : delta_grown_elements) {
        ASSERT_EQ(delta_grown_element,
            0.0)
            << ss.
...
TEST_F(SynapticElementsTest, testInitialElementsConstant) {
    ...
    for (
        ...
        ASSERT_EQ(grown_elements_1, grown_elements_2) << ss.

                                                         str()

                                                      << neuron_id;
        ASSERT_EQ(grown_elements_1, bound) << ss.
...
TEST_F(SynapticElementsTest, testSignalTypes) {
    ...
    for (
        ...
        ASSERT_EQ(synaptic_elements
            ...
            golden_signal_types[neuron_id.get_neuron_id()])
            << ss.
...
TEST_F(SynapticElementsTest, testHistogram) {
    ...
    for (const auto& [pair, count] : histogram) {
        ...
        ASSERT_EQ(count, golden_count) << ss.
    ...
    for (
        ...
        ASSERT_THROW(synaptic_elements
                         .update_connected_elements(neuron_id,
                             static_cast<int>(connected_grown_element)),
            RelearnException)
            << ss.
...
TEST_F(SynapticElementsTest, testMultipleUpdate) {
    ...
    for (
        auto neuron_id :
        NeuronID::range(number_neurons)) {
        ASSERT_EQ(golden_counts[neuron_id.get_neuron_id()], synaptic_elements.get_grown_elements(neuron_id)) << ss.

                                                                                                                str()

                                                                                                             << neuron_id;
        ASSERT_EQ(golden_counts[neuron_id.get_neuron_id()], grown_elements[neuron_id.get_neuron_id()]) << ss.

                                                                                                          str()

                                                                                                       << neuron_id;

        ASSERT_EQ(golden_connected_counts[neuron_id.get_neuron_id()], synaptic_elements.get_connected_elements(neuron_id)) << ss.

                                                                                                                              str()

                                                                                                                           << neuron_id;
        ASSERT_EQ(golden_connected_counts[neuron_id.get_neuron_id()], connected_grown_elements[neuron_id.get_neuron_id()]) << ss.

                                                                                                                              str()

                                                                                                                           << neuron_id;

        ASSERT_EQ(golden_signal_types[neuron_id.get_neuron_id()], synaptic_elements.get_signal_type(neuron_id)) << ss.

                                                                                                                   str()

                                                                                                                << neuron_id;
        ASSERT_EQ(golden_signal_types[neuron_id.get_neuron_id()], signal_types[neuron_id.get_neuron_id()]) << ss.
...
TEST_F(SynapticElementsTest, testFreeElements) {
    ...
    for (
        ...
        ASSERT_EQ(expected_number_free_elements_cast, synaptic_elements.get_free_elements(neuron_id)) << ss.
        ...
        if (golden_signal_types[nid] == SignalType::Excitatory) {
            ASSERT_EQ(expected_number_free_elements_cast, synaptic_elements.get_free_elements(neuron_id, SignalType::Excitatory)) << ss.

                                                                                                                                     str()

                                                                                                                                  << neuron_id;
            ASSERT_EQ(0, synaptic_elements.get_free_elements(neuron_id, SignalType::Inhibitory)) << ss.

                                                                                                    str()

                                                                                                 << neuron_id;
        } else {
            ASSERT_EQ(expected_number_free_elements_cast, synaptic_elements.get_free_elements(neuron_id, SignalType::Inhibitory)) << ss.

                                                                                                                                     str()

                                                                                                                                  << neuron_id;
            ASSERT_EQ(0, synaptic_elements.get_free_elements(neuron_id, SignalType::Excitatory)) << ss.
...
TEST_F(SynapticElementsTest, testDisable) {
    ...
    for (
        ...
        if (is_disabled) {
            ...
            ASSERT_EQ(synaptic_elements
                          .get_connected_elements(neuron_id),
                golden_connected_counts[neuron_id.get_neuron_id()])
                << ss.
            ...
            ASSERT_EQ(synaptic_elements
                          .get_grown_elements(neuron_id),
                golden_counts[neuron_id.get_neuron_id()])
                << ss.
...
TEST_F(SynapticElementsTest, testUpdateNumberElements) {
    ...
    for (
        ...
        if (disable_flags[id.
            ...
            ASSERT_NEAR(actual_delta, computed_delta, eps) << ss.

                                                              str()

                                                           << id;
            ASSERT_NEAR(actual_deltas[id.get_neuron_id()], computed_delta, eps) << ss.
...
TEST_F(SynapticElementsTest, testMultipleUpdateNumberElements) {
    ...
    for (
        ...
        ASSERT_NEAR(actual_delta, expected_delta, eps) << ss.

                                                          str()

                                                       << neuron_id;
        ASSERT_NEAR(actual_deltas[neuron_id.get_neuron_id()], expected_delta, eps) << ss.
...
TEST_F(SynapticElementsTest, testCommitUpdates) {
    ...
    for (
        ...
        ] = static_cast<unsigned int>(connected_grown_element);
        golden_signal_types[neuron_id.
        ...
        synaptic_elements.update_grown_elements(neuron_id, grown_element);
        synaptic_elements.update_connected_elements(neuron_id,
            static_cast<int>(connected_grown_element));
        synaptic_elements.set_signal_type(neuron_id, signal_type);
    }
    ...
    for (
        ...
        if (disable_flags[neuron_id.
            ...
            ASSERT_NEAR(delta, computed_delta, eps) << ss.

                                                       str()

                                                    << neuron_id;
            ASSERT_NEAR(deltas[neuron_id.get_neuron_id()], computed_delta, eps) << ss.
    ...
    for (
        auto deleted_counts : deleted_element_counts) {
        summed_number_deletions += deleted_counts;
    }

    ASSERT_EQ(summed_number_deletions, number_deleted_elements) << ss.

                                                                   str()

                                                                << summed_number_deletions << ' ' << number_deleted_elements;
    ...
    for (
        ...
        if (new_vacant >= 0.0) {
            ...
            ASSERT_NEAR(expected_count, current_count, eps) << ss.

                                                               str()

                                                            << neuron_id;
            ASSERT_EQ(previous_connected, current_connected) << ss.
        ...
        if (expected_deletions == previous_connected) {
            ...
            ASSERT_NEAR(current_count, expected_count, eps) << ss.
        ...
        ASSERT_EQ(expected_connected, current_connected) << ss.

                                                            str()

                                                         << neuron_id;
        ASSERT_EQ(expected_deletions, deleted_element_counts[neuron_id.get_neuron_id()]) << ss.
