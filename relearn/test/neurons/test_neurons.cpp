...
TEST_F(NeuronsTest, testStaticConnectionsChecker) {
    ...
    for (
        ...
        do {
            ...
            ranges::contains(static_neurons, src) || ranges::contains(static_neurons, tgt) || src == tgt);
...
TEST_F(NeuronsTest, testDisableNeuronsWithoutMPI) {
    ...
    ASSERT_THROW(neurons
                     ->enable_neurons(enabled_neurons),
        RelearnException);
    ...
    ASSERT_THROW(neurons
                     ->disable_neurons(1, disabled_neurons, 1),
        RelearnException);
    ...
    for (
        ...
        ASSERT_EQ(neurons
                      ->get_extra_info()
                      ->get_disable_flags()[id.get_neuron_id()],
            UpdateStatus::Enabled);
    }

    for (
        const auto& id : in_ids) {
    ...
    ASSERT_EQ(neurons
                  ->get_extra_info()
                  ->get_disable_flags()[disable_id.get_neuron_id()],
...
TEST_F(NeuronsTest, testDisableMultipleNeuronsWithoutMPI) {
    ...
    for (
        const auto& neuron1 : disabled_neurons) {
        for (
            const auto& neuron2 : disabled_neurons) {
            if (neuron1 == neuron2) {
                continue;
            }
    ...
    to_delete_axons.resize(num_neurons,
        0.0);
    to_delete_den_ex.resize(num_neurons,
        0.0);
    to_delete_den_inh.resize(num_neurons,
        0.0);
    int expected_num_deletions = 0;
    for (
        const auto& disabled_id : disabled_neurons) {
    ...
    ASSERT_EQ(num_distant_deletions,
        0);
    for (
        const auto& disable_id : disabled_neurons) {
...
TEST_F(NeuronsTest, testDisableNeuronsWithRanks) {
    ...
    expected_distant_out_deletions_received.resize(num_ranks,
        0);
    expected_distant_in_deletions_received.resize(num_ranks,
        0);
    std::vector<size_t> expected_distant_out_deletions_initiated{};
    std::vector<size_t> expected_distant_in_deletions_initiated{};
    expected_distant_out_deletions_initiated.resize(num_ranks,
        0);
    expected_distant_in_deletions_initiated.resize(num_ranks,
        0);
    for (
        int rank = 0;
        rank < num_ranks;
    ...
    std::vector<std::vector<SignalType>> signal_types = rank_to_neurons | ranges::views::transform([](const auto& neurons_of_rank) {
    ...
    for (
        ...
        for (
            const auto neuron_id : rank_to_disabled_neurons[rank]) {
            ...
            for (
                const auto& source : distant_in_edges | ranges::views::keys) {
            ...
            for (
                const auto& source : in_edges | ranges::views::keys) {
    ...
    for (
        ...
        for (
            ...
            ASSERT_EQ(neurons
                      ...
                      get_disable_flags()[neuron_id.get_neuron_id()],
                UpdateStatus::Enabled
...
TEST_F(NeuronsTest, testDisableNeuronsWithRanksAndOnlyOneDisabledNeuron) {
    ...
    for (
        const auto& source : local_in_edges | ranges::views::keys) {
