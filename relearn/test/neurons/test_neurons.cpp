...
set_axons(axs);
sdf->
set_dendrites_ex(dends_ex);
sdf->
set_dendrites_in(dends_in);

Neurons neurons{partition, std::move(model), std::move(calcium), std::move(network_graph), std::move(axs),
                std::move(dends_ex),
...
const auto signal_types =
        ranges::views::generate_n([this]() { return NeuronTypesAdapter::get_random_signal_type(mt); }, num_neurons)
        | ranges::to_vector;

for (
int synapse_nr = 0;
synapse_nr<num_synapses;
synapse_nr++) {
...
network_graph->
add_synapse(StaticLocalSynapse{tgt, src, weight}
);
} else {
network_graph->
add_synapse(StaticDistantOutSynapse{RankNeuronId(tgt_rank, tgt), src, weight}
);
}
}
...
for (
int test_synapse = 0;
test_synapse<num_test_synapses;
test_synapse++) {
const auto src = NeuronIdAdapter::get_random_neuron_id(num_neurons, mt);
const auto tgt_rank = MPIRankAdapter::get_random_mpi_rank(num_ranks, mt);
const auto tgt = NeuronIdAdapter::get_random_neuron_id(num_neurons, src, mt);
...
network_graph->
add_synapse(PlasticLocalSynapse{tgt, src, weight}
);
} else {
network_graph->
add_synapse(PlasticDistantOutSynapse{RankNeuronId(tgt_rank, tgt), src, weight}
);
}
...
const auto i
:
NeuronID::range_id(num_static_neurons)
) {
NeuronID static_neuron;
do {
static_neuron = NeuronIdAdapter::get_random_neuron_id(num_neurons, mt);
} while (
ranges::contains(static_neurons, static_neuron
));
static_neurons.
emplace_back(static_neuron);
}
...
auto model = std::make_unique<models::PoissonModel>(models::PoissonModel::default_h,
                                                    std::make_unique<LinearSynapticInputCalculator>(
                                                            ...
                                                            std::make_unique<FiredStatusCommunicationMap>(1)),
...
[](MPIRank /*mpi_rank*/, NeuronID::value_type /*neuron_id*/) {
return 0.0; });
...
const auto i
:
ranges::views::indices(num_synapses_static)
) {
...
const auto i
:
ranges::views::indices(num_synapses_plastic)
) {
NeuronID src, tgt;
do {
...
ranges::contains(static_neurons, src
) ||
ranges::contains(static_neurons, tgt
) || src == tgt);
const auto weight = std::abs(NetworkGraphAdapter::get_random_plastic_synapse_weight(mt));
network_graph->
add_synapse(PlasticLocalSynapse{NeuronID{tgt}, NeuronID{src}, weight}
);
}

neurons.
set_static_neurons(static_neurons);

const auto num_tries = RandomAdapter::get_random_integer(10, 100, mt);
for (
const auto i
:
ranges::views::indices(num_tries)
) {
...
ASSERT_THROW(neurons
.
set_static_neurons(static_neurons), RelearnException
);
network_graph->
add_synapse(PlasticLocalSynapse{tgt, src, -weight}
);
...
neurons->
init(num_neurons);
NetworkGraphAdapter::create_dense_plastic_network(network_graph, neurons
->
...
ASSERT_THROW(neurons
->
enable_neurons(enabled_neurons), RelearnException
);

const auto &[num_deletions, synapse_deletion_Requests] = neurons->disable_neurons(1, disabled_neurons, 1);
ASSERT_EQ(synapse_deletion_Requests
.

get_total_number_requests(),

0);
ASSERT_EQ(num_deletions, out_edges
.
...
ASSERT_THROW(neurons
->disable_neurons(1, disabled_neurons, 1), RelearnException);

ASSERT_EQ(out_edges_ref
.

size(),

0);
ASSERT_EQ(in_edges_ref
.
...
ASSERT_EQ(neurons
->get_extra_info()->get_disable_flags()[id.get_neuron_id()], UpdateStatus::Enabled);
}
...
.
get_connected_elements(id),
7);
}

ASSERT_EQ(neurons
->get_extra_info()->get_disable_flags()[disable_id.get_neuron_id()], UpdateStatus::Disabled);
ASSERT_EQ(neurons
->
...
.
get_connected_elements(disable_id),
0);
ASSERT_EQ(neurons
->
...
NetworkGraphAdapter::check_validity_of_network_graphs({
network_graph }, signal_types, num_neurons);
}
...
std::vector <RankNeuronId> disabled_rank_neurons = disabled_neurons | ranges::views::transform(
        [&mpi_rank](const auto &neuron_id) { return RankNeuronId(mpi_rank, neuron_id); }) | ranges::to_vector;
for (
const auto &neuron1
: disabled_neurons) {
for (
...
add_synapse(PlasticLocalSynapse(neuron2, neuron1, weight)
);
}
}
NetworkGraphAdapter::create_dense_plastic_network(network_graph_plastic, neurons
->
...
if (weight > 0) {
to_delete_den_ex[target.

get_neuron_id()

]++;
} else {
to_delete_den_inh[target.
...
0);
for (
const auto &disable_id
: disabled_neurons) {
ASSERT_EQ(std::get<0>(network_graph_plastic->get_local_in_edges(disable_id))
.

size(),

0);
ASSERT_EQ(std::get<0>(network_graph_plastic->get_local_out_edges(disable_id))
.
...
get_connected_elements(neuron_id),
        axons_old[neuron_id.get_neuron_id()]
- to_delete_axons[neuron_id.
...
.
get_connected_elements(neuron_id),
        den_ex_old[neuron_id.get_neuron_id()]
- to_delete_den_ex[neuron_id.
...
.
get_connected_elements(neuron_id),
        den_inh_old[neuron_id.get_neuron_id()]
- to_delete_den_inh[neuron_id.
...
const std::vector <std::vector<SignalType>> signal_types{neurons->get_axons().get_signal_types() | ranges::to_vector};
NetworkGraphAdapter::check_validity_of_network_graphs({
network_graph_plastic }, signal_types, num_neurons);
}
...
expected_distant_out_deletions_received.
resize(num_ranks,
0);
expected_distant_in_deletions_received.
resize(num_ranks,
0);
std::vector <size_t> expected_distant_out_deletions_initiated{};
std::vector <size_t> expected_distant_in_deletions_initiated{};
expected_distant_out_deletions_initiated.
resize(num_ranks,
0);
expected_distant_in_deletions_initiated.
resize(num_ranks,
0);
for (
int rank = 0;
...
8, num_ranks, mpi_rank, mt);

Neurons::check_signal_types(network_graph_plastic, neurons
->
...
get_signal_types(), mpi_rank

);

rank_to_neurons.
push_back(std::move(neurons)
);
rank_to_disabled_neurons.
push_back(disabled_neurons);
network_graphs.
push_back(network_graph_plastic);
}
NetworkGraphAdapter::harmonize_network_graphs_from_different_ranks(network_graphs, num_neurons
);

std::vector <std::vector<SignalType>> signal_types = rank_to_neurons | ranges::views::transform(
        [](const auto &neurons_of_rank) {
            return neurons_of_rank->get_axons().get_signal_types() | ranges::to_vector;
        }) | ranges::to_vector;
...
const auto den_ex = neurons->get_dendrites_exc().get_connected_elements();
const auto den_inh = neurons->get_dendrites_inh().get_connected_elements();
expected_axons[rank] = axons |
ranges::to_vector;
expected_den_ex[rank] = den_ex |
ranges::to_vector;
expected_den_inh[rank] = den_inh |
ranges::to_vector;
}
...
for (
const auto neuron_id
: rank_to_disabled_neurons[rank]) {
const auto &[distant_out_edges, _3] = network_graph->get_distant_out_edges(neuron_id);
expected_distant_out_deletions_initiated[rank] += distant_out_edges.

size();

for (const auto &[target, weight] : distant_out_edges) {
expected_distant_in_deletions_received[target.
...
expected_distant_out_deletions_initiated[rank] += distant_in_edges.

size();

for (
const auto &source
: distant_in_edges | ranges::views::keys) {
expected_distant_out_deletions_received[source.
...
for (const auto &[target, weight] : out_edges) {
if (weight > 0) {
expected_den_ex[rank][target.

get_neuron_id()

]--;
} else {
expected_den_inh[rank][target.
...
for (
const auto &source
: in_edges | ranges::views::keys) {
expected_axons[rank][source.
...
ASSERT_EQ(num_deletions, expected_num_local_deletions
+ expected_distant_out_deletions_initiated[rank] + expected_distant_in_deletions_initiated[rank]);

ASSERT_FALSE(synapse_deletion_Requests
.
contains(MPIRank(rank)
));

outgoing_requests.
push_back(synapse_deletion_Requests);

for (
const auto &disable_id
: disabled_neurons) {
ASSERT_EQ(std::get<0>(network_graph->get_local_in_edges(disable_id))
.

size(),

0);
ASSERT_EQ(std::get<0>(network_graph->get_local_out_edges(disable_id))
.

size(),

0);
ASSERT_EQ(std::get<0>(network_graph->get_distant_in_edges(disable_id))
.

size(),

0);
ASSERT_EQ(std::get<0>(network_graph->get_distant_out_edges(disable_id))
.
...
0);
ASSERT_EQ(neurons
->get_extra_info()->get_disable_flags()[disable_id.get_neuron_id()],
...
const auto &num_distant_deletions = neurons->delete_disabled_distant_synapses(synapse_deletion_Requests,
                                                                              mpi_rank);
ASSERT_EQ(num_distant_deletions,
        expected_distant_in_deletions_received[rank]
+ expected_distant_out_deletions_received[rank]);
for (
const auto &disable_id
: disabled_neurons) {
ASSERT_EQ(std::get<0>(network_graph->get_local_in_edges(disable_id))
...
get_disable_flags()[neuron_id.get_neuron_id()], UpdateStatus::Enabled
...
const auto &[distant_out_edges, _3] = network_graphs[disabled_rank.get_rank()]->get_distant_out_edges(
        disabled_neuron_id);
for (const auto &[target, weight] : distant_out_edges) {
if (weight > 0) {
expected_den_ex[target.
...
expected_den_inh[disabled_rank.
...
for (
const auto &source
: local_in_edges | ranges::views::keys) {
expected_axons[disabled_rank.
...
== rank) {
disabled_neurons_vector.
push_back(disabled_neuron_id);
}
auto &neurons = rank_to_neurons[rank];
...
if (rank == disabled_rank.

get_rank()

) {
ASSERT_EQ(num_deletions, num_local_deletions
+ num_distant_deletions);
ASSERT_EQ(synapse_deletion_Requests
.

get_total_number_requests(), num_distant_deletions

);
} else {
ASSERT_EQ(num_deletions,
0);
ASSERT_EQ(synapse_deletion_Requests
.
...
std::tuple <std::shared_ptr<Neurons>, std::shared_ptr<NetworkGraph>>
NeuronsTest::create_neurons_object(std::shared_ptr <Partition> &partition, MPIRank rank) {
    auto model = std::make_unique<models::PoissonModel>(models::PoissonModel::default_h,
                                                        std::make_unique<LinearSynapticInputCalculator>(
                                                                SynapticInputCalculator::default_conductance,
                                                                std::make_unique<FiredStatusCommunicationMap>(1)),
    ...
    auto neurons = std::make_shared<Neurons>(partition, std::move(model), std::move(calcium), network_graph,
                                             std::move(axs), std::move(dends_ex), std::move(dends_in), std::move(sdf));
