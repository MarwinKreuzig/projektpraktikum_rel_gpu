...
ASSERT_THROW(Partition
part(0, my_rank), RelearnException) <<
my_rank;
}
...
if (!is_rank_power_2) {
ASSERT_THROW(Partition
part(num_ranks, my_rank
), RelearnException) << num_ranks << ' ' <<
my_rank;
return;
}
...
const auto num_subdomains_per_dim = static_cast<size_t>(std::ceil(
        std::pow(static_cast<double>(num_subdomains), 1.0 / 3.0)));
...
get_number_mpi_ranks(), num_ranks

);
ASSERT_EQ(partition
.

get_my_mpi_rank(), my_rank
...
get_total_number_subdomains(), num_subdomains
...
get_number_local_subdomains(), my_subdomains
...
get_local_subdomain_id_start(), my_subdomains
...
get_local_subdomain_id_end(), my_subdomains
...
get_level_of_subdomain_trees(), oct_exponent
...
get_number_subdomains_per_dimension(), num_subdomains_per_dim

) <<
num_subdomains_per_dim;
}
}
...
const auto my_rank
:
ranges::views::indices(num_ranks)
) {
number_local_neurons[my_rank] =
std::vector<size_t>(my_subdomains);

for (
const auto my_subdomain
:
ranges::views::indices(my_subdomains)
) {
const auto num_local_neurons = NeuronIdAdapter::get_random_number_neurons(mt);

number_local_neurons[my_rank][my_subdomain] =
num_local_neurons;
number_total_neurons +=
num_local_neurons;
}
}
...
partition.
set_total_number_neurons(number_total_neurons);
ASSERT_EQ(partition
.

get_total_number_neurons(), number_total_neurons

);

const auto num_local_neurons = ranges::accumulate(local_neurons, size_t{0});
partition.
set_number_local_neurons(num_local_neurons);

ASSERT_EQ(partition
.

get_number_local_neurons(), num_local_neurons
...
ASSERT_EQ(index_1, translated_index_1
) << index_1 << ' ' <<
translated_index_1;
ASSERT_EQ(index_3, translated_index_3
) << index_3 << ' ' <<
translated_index_3;

ASSERT_FALSE(found_indices[index_1])
<<
index_1;
ASSERT_TRUE(index_1<num_subdomains)
<< index_1 << ' ' <<
num_subdomains;
...
ASSERT_THROW(partition
.
set_simulation_box_size(simulation_box_maximum, simulation_box_minimum
), RelearnException);

ASSERT_THROW(partition
.set_simulation_box_size({
simulation_box_minimum.

get_x()

+ Constants::uninitialized, simulation_box_minimum.

get_y(), simulation_box_minimum

.

get_z()

}, simulation_box_maximum), RelearnException);
ASSERT_THROW(partition
.set_simulation_box_size({
simulation_box_minimum.

get_x(), simulation_box_minimum
...
+ Constants::uninitialized }, simulation_box_maximum), RelearnException);

ASSERT_THROW(partition
.
set_simulation_box_size(simulation_box_minimum,
{
simulation_box_maximum.

get_x()

+ Constants::uninitialized, simulation_box_maximum.

get_y(), simulation_box_maximum
...
+ Constants::uninitialized }), RelearnException);
...
partition.
set_simulation_box_size(simulation_box_minimum, simulation_box_maximum
);
...
for (
size_t subdomain_index_1 = 0;
subdomain_index_1<num_subdomains;
subdomain_index_1++) {
...
ASSERT_EQ(min_1, min_3
) << min_1 <<
min_3;
ASSERT_EQ(max_1, max_3
) << max_1 <<
max_3;
...
ASSERT_NEAR(difference_min
.calculate_p_norm(1.0), 0.0, eps) << min_1 <<
subdomain_expected_min;
ASSERT_NEAR(difference_max
.calculate_p_norm(1.0), 0.0, eps) << max_1 <<
subdomain_expected_max;
}

partition.

calculate_and_set_subdomain_boundaries();
...
local_subdomain_boundaries.
emplace_back(min, max
);
}
...
ASSERT_THROW(auto val = partition.get_subdomain_boundaries(my_subdomain + num_subdomains), RelearnException)
...
const auto j
:
ranges::views::indices(iterations)
) {
const auto &position = SimulationAdapter::get_random_position_in_box(simulation_box_minimum, simulation_box_maximum,
                                                                     mt);
...
for (
auto subdomain_id = index_1_start;
subdomain_id<index_1_start +
my_subdomains;
subdomain_id++) {
