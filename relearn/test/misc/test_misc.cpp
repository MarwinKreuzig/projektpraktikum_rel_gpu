...
const auto val
:
ranges::views::iota(integer_type{0}, integer_type{10}
)) {
ASSERT_EQ(1,
Util::num_digits(val)
);
...
ASSERT_THROW(auto val = Util::min_max_acc(std::span<const size_t>{{10, 422, 5223, 554315}}, extra_infos),
             RelearnException);
...
ASSERT_THROW(auto val = Util::min_max_acc(std::span<const float>{{0.8f, -1.6f, 65423.8f}}, extra_infos),
             RelearnException);
...
const auto random_value = RandomAdapter::get_random_integer<size_t>(std::numeric_limits<size_t>::min(),
                                                                    std::numeric_limits<size_t>::max(), mt);
