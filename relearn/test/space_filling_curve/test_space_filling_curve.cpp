...
TEST_F(SpaceFillingCurveTest, testMortonConstructor) {
    ...
    for (
        ...
        refinement_level < SimulationAdapter::max_refinement_level;
        refinement_level++) {
        std::stringstream ss{};
        ...
        ASSERT_EQ(refinement_level, res) << ss.
...
TEST_F(SpaceFillingCurveTest, testMortonTranslationBruteForce) {
    ...
    for (
        auto refinement_level = 0;
        refinement_level < SimulationAdapter::small_refinement_level;
        refinement_level++) {
        morton.set_refinement_level(refinement_level);
        ...
        for (
            size_t x = 0;
            x < num_boxes_per_dimension;
            x++) {
            for (
                size_t y = 0;
                y < num_boxes_per_dimension;
                y++) {
                for (
                    size_t z = 0;
                    z < num_boxes_per_dimension;
                    z++) {
                    Vec3s index3d{ x, y, z };
                    ...
                    ASSERT_EQ(index3d, res) << ss.

                                               str();

                    ASSERT_LT(index1d, total_num_boxes) << ss.
...
TEST_F(SpaceFillingCurveTest, testSpaceFillingCurveMortonConstructor) {
    ...
    ASSERT_EQ(refinement_level_2, res2) << ss.
...
TEST_F(SpaceFillingCurveTest, testSpaceFillingCurveMortonConstructorException) {
    ...
    ASSERT_THROW(SpaceFillingCurve<Morton>
                     sfc(cast_refinement_level);
                 , RelearnException)
        << ss.
...
TEST_F(SpaceFillingCurveTest, testSpaceFillingCurveMortonSetRefinementException) {
    ...
    ASSERT_THROW(sfc
                     .set_refinement_level(cast_refinement_level);
                 , RelearnException)
        << ss.
