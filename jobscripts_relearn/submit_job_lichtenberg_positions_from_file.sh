...
if test "$#" -ne 6; then
    echo "Usage: $0 <num procs> <num nodes> <num neurons> <simulation steps> <num repetitions> <file with neuron positions>"
    exit
fi
...
file_with_neuron_positions_modified=${file_with_neuron_positions////_}

home=/home/sr42myny/RELEARN/Klinikum-TUM
bindir=$home/src.git/relearn/
outputdir=$home/joboutput
prefix="relearn"

name=${prefix}_procs-${num_procs}_neurons-${num_neurons}_steps-${num_sim_steps}_neuronPositions-${file_with_neuron_positions_modified}
job=${name}.sbatch
